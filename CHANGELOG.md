...
- `dna/core/workflow_runtime.hl`, `dna/core/workflow_execution.hl`: an execution restored after a restart (`ExecutionAsked` with `restore`) proposes its transitions as a fresh one does and the committer replays what the record holds, so every id is the record's and a completed record rebooted gains no row and runs nothing. A restored Work's first request asks for redelivery (`AttemptRequest.redeliver`), which the runtime decides once per incarnation per attempt through `redeliver_attempt`: an outcome recorded is the answer and the open claim the dead incarnation left under it is closed; an attempt never claimed runs the ordinary path; one claimed and never answered is dispatched again with the same attempt id and no new claim — a transport redelivery, not a new attempt — unless the execution was cancelled; a second request attaches, so a duplicate dispatch starts no second invocation; a reply from the old process for the still-current attempt is accepted. `workflow_restore_test.hl` runs a flat one-leaf workflow once, cuts its record before the attempt's admission, after it, after the claim, after the outcome and after completion, and boots a fresh owner and runtime over each cut in the same scope (the previous incarnation dissolved): every boot reaches the same completed record with the same ids, the performer is asked exactly when the record says nothing answered, a duplicate dispatch while the redelivered attempt is pending attaches, the old process's reply settles it, and two reboots of the completed record write and run nothing. One flat leaf, no retries, an effect-free performer.
