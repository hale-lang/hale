...
- **The whole-process drain ships.** The spec's drain cascade was
  ...
  or SIGTERM makes `self.draining` true for every locus, returns
  `sleep`s and timed `async_io` receives early, lets each watching
  `run()` end and dissolve leaves-first, and exits 0. A drain that has
  not finished within 5 s (`LOTUS_DRAIN_GRACE_MS`) exits `128 +
  signal`, as does a second signal. A program that never reads the
  flag keeps the default action.
- **`main`'s `run()` ending does not end the program** while a child's
  `run()` still loops; the spec now says so, and SIGTERM is how such a
  program is stopped.
...
This is the DNA prerelease. An organization written in Hale now has a workflow, three memories with owners, funding and leases, a record with an API of its own and the protections a business needs, and a host that runs the body on a server and fences it with a lease. It is a prerelease because the shapes are still moving under fixtures rather than under customers; the DNA book describes what holds today. Underneath it the compiler stopped letting `hale check` and `hale build` disagree, locus ownership is decided by a table before any code is emitted, and every command renders a diagnostic at the span the author wrote.

The item-level notes for this release — one entry per change, with every issue reference — are the CHANGELOG as it stood at the tag: `git show v0.21.0:CHANGELOG.md`.

### DNA

- **The workflow.** Fifteen durable-first cards: nothing runs before the record holds the fact that it may. A routed retry carries its own attempt number (01), an execution is admitted atomically under its limits (07), exactly one attempt is ever admitted (08), one step stays alive across delayed replies (09), ordered steps advance only behind committed completions (10), child workflows recurse with one engine at every level (11), a restart redelivers what was admitted, fences the old incarnation and reconciles uncertain external effects (12a–12c), a tree is rebuilt from the record alone (13), each Work resumes under its own plan (14), and an edit's outcome comes back for the exact attempt that asked (15).
- **Owners, ledger, funding and leases (GH #646 stages 0–5, #664–#669).** Three memories, one home per row kind: the Structure, the record and the ledger. `hale dna ledger adopt` moves the day's work into the store; heads write through the service and queue what they cannot deliver; evidence and exports read both memories as one; handoffs go service to service. Over a shared record each owner has its positions, its lease and epoch, its id namespace, its transfers and its funding, and the host names who serves the record.
- **The record.** A reconcile never rewinds the record and an append never gives up (GH #603). The record has an API whose vocabulary never names git (GH #649), a Task's birth is in the record before anything runs (GH #604), and one body per record holds a lease at the remote (GH #617). Protected evidence and retention (GH #606), the prompt as a receipt (GH #611), schedules (GH #610), grants over money, counterparties, routes and time (GH #605), a knowledge store scoped by its record (GH #613), cross-record handoff (GH #615), attributed external decisions (GH #616) and a principal source that signs you in (GH #612). Each request is answered by its own answer (GH #689), a publish the membrane loses is not a lost fact (GH #682), and a concern's occurrence survives a failed read of the record (GH #748).
- **Knowledge, membrane and hosting.** A body's infrastructure and transport are implementations behind the core's interfaces (GH #647); the GitHub membrane sits behind a `Forge` interface (GH #648); a person can propose a practice for the Board (GH #602). Fixtures take a port they have bound and clean their scratch roots (GH #873, #909); the suite's guards blame only their own slice (GH #872, #795).

### Language and checker

- **`Time` is a value (GH #607).** Nanoseconds since the Unix epoch, with a backticked ISO-8601 literal, `Time ± Duration`, `Time − Time`, ordering, and `std::time::current()`, `iso8601`, `parse_time`, `unix`, `nanos` and `from_nanos`.
- **Check and build read one vocabulary (GH #911).** The strict-identifier rule applies wherever the compiler has the whole program, with single-file `hale check` keeping its permissive reading (B1). An unresolvable qualified name is a located error with two messages, unknown head and unknown name (B2). A generic instantiation typechecks as the monomorph codegen builds; un-annotated and wrong-arity literals are refused where they are written (B5). Generic-argument tokens and statement-position builtins come from one shared table (B3). A file belongs to exactly one library, `target` is program-level and `fn main` is top-level (B6). A param default that constructs its own locus type is refused (B4).
- **A module is not a namespace, and every check looks inside one (GH #825, #764, #884, #854).** Twelve bundle-level checks and codegen used to stop at `module { }`; they walk it now, and a module-nested declaration is reached across an import like a top-level one.
- **A builtin never silently beats a declaration you wrote (GH #880, #863, #892),** an unknown bare type name (GH #877), a self-containing locus (GH #813), a pinned locus in a loop (GH #826), a dropped placement entry (GH #890), a bus payload the bus cannot carry (GH #876), an undefined identifier (GH #721), a reserved word used as a name (GH #725) and a `type` or `const` inside a locus (GH #756, #747) are all located errors instead of build failures or silent misbehaviour.
- **Imports and aliases resolve the same way at check and at build:** one library under two spellings is one library (GH #763), a qualified path must use an alias its own seed declares (GH #762), an alias is scoped to its seed (GH #746), an imported type in annotation position (GH #833), a struct literal spelled with an alias (GH #831), the `type Name = Type;` form (GH #759), claims through an import (GH #774, #733), and `serves` with a qualified perspective (GH #724). A library that fails to parse fails `check` and `verify` (GH #765).
- **Diagnostics know the stdlib:** conventional spellings point at the builtin (GH #722), stdlib argument types and struct-returning helpers are typed (GH #829, #771), the blocking lints read the effects registry and know `sleep` parks on `async_io` (GH #830, #791), and `@unbounded` stacks with the effect assertions (GH #723).

### Locus ownership

- **Who reclaims a locus is decided before lowering (GH #921).** A pre-pass computes an owner for every locus-producing expression; the seven one-shot flags are gone and an instantiation with no owner is a hard build error. A generated matrix of 945 cells holds every position, type and context under four oracles.
- **Every ownership leak the sweep found is closed:** a factory result owned by the node the site names (GH #837), per path through `if` / `match` / a block (GH #883), a `let`-bound factory dissolved by its scope (GH #793), a factory into a param field in every spelling (GH #836, #853, #895), a loop reclaiming every iteration (GH #815), a literal in expression position living as long as its caller (GH #711, #812, #710), the teardown cascade reaching the leaves (GH #750), every owned child arena destroyed (GH #871), a `return` in `main` no longer leaking (GH #789), and the bindings transport dissolved at exit (GH #893).

### Runtime, codegen and standard library

- A cooperative handler owns its payload across a park (GH #781); a message queued for a locus that ended reaches nobody (GH #703); `LOTUS_NO_CHUNK_POOL` lets the sanitizer see arena use-after-free (GH #816); a non-escaping `[c; N]` is a stack slot (GH #767); libm is always linked and a missing tree-sitter shim is a located build error (GH #808); `hale test` dissolves the test's loci before it exits (GH #717); the DNA host builds the organism's seed once per content fingerprint (PR #931).
- A byte view for linear-time scanning of large Strings (GH #720); `std::json::string_field` tells null from missing (GH #719); `std::json::valid` and `valid_object` (GH #754); JSON Unicode escapes decode and escaping is linear (GH #708, #709); `std::process::adopt` moves a spawned `Child` into a locus field (GH #716).

### CLI, LSP, diagnostics and replay

- `hale run` and `hale replay` take the build options and fingerprint them (GH #904); an observed iris session dies with `hale` and owns its stdout (GH #905); a stdlib-origin span renders as a note, never a seed range (GH #856); an imported enum prints its declared name (GH #885); no shipped code mutates the process environment (GH #887).
- Every command renders a codegen error at its location (GH #848), spells a diagnostic's file canonically (GH #822) and positions an imported file's diagnostic in its own file (GH #775); `--help` works on every subcommand (GH #817) and usage lists every one (GH #861); `check --json` records unreadable inputs, parse errors and unresolvable imports (GH #806, #777, #860); the LSP publishes a diagnostic at a file's end (GH #805); `hale --version` names the embedded DNA core's digest (GH #726).
- Replay's coverage report says what it compared for in-process payloads (GH #842, codec to follow in GH #947), direct-dispatch publishes record their payloads (GH #782), and a match names every category it compared (GH #728).

### Tests and CI

- `cargo test` agrees with nextest, and a `parity` job keeps both shapes honest (GH #912); no test mutates the process environment (GH #843); RSS regression tests assert growth shape (GH #772).
