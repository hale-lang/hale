...
- **Only `ok` dispatches.** On a refusal the proposer dispatches
  nothing.
- **The committer is fenced on its lease** (card 12b): with a lease
  key, every commit, claim, redelivery and recorded outcome is decided
  on the live fencing token; a stale holder answers `fenced`, which a
  proposer holds as it holds `record` and proposes again when the
  record resumes under a live token. **[proven, card 12b]**
