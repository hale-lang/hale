...
| `std::bytes` | `at(b, i) -> Int fallible(IndexError)`, `slice(b, lo, hi)`, `from_string(s)`, `from_int(b)`, `concat(a, b)`, `builder_new` / `builder_append(b, chunk)` / `builder_len(b)` / `builder_finish(b)` — binary-safe sibling of the `std::str::builder_*` family; the append reads each chunk's length prefix so embedded NULs survive, finish emits a length-prefixed Bytes blob without a trailing NUL. **In-place ops** (added 2026-05-19): `builder_shift_front(b, n)` drops the first n bytes via memmove (capacity preserved); `builder_clear(b)` sets len=0; `builder_snapshot(b) -> Bytes` copies current contents into a fresh Bytes blob without disturbing the builder; `builder_free(b)` disposes the builder without materializing a final blob. Together they let a long-lived holder recycle one allocation across many iterations (recv-loop accumulators, framing buffers) — without them, `concat`/`slice` churn a fresh allocation per iteration. **Note:** builder handles are typed as `Bytes` for return-value convenience but are NOT layout-compatible with regular Bytes blobs — `std::bytes::at(builder, i)` / `len(builder)` read the wrong slots. Use `builder_snapshot(b)` to materialize a regular Bytes view, or `builder_len(b)` for the running length. Literal syntax: `b"..."` (added 2026-05-17) — same escapes as String, `\xNN` accepts full 0x00..0xFF; NUL-safe; length carried alongside the buffer. |
...
| `std::io::tcp` | path-call entry points `listen_socket(host, port) -> Int fallible(IoError)`, `connect(host, port) -> Int fallible(IoError)` (accepts dotted-quad hosts directly and falls back to hostname resolution via `getaddrinfo` (AF_INET) when the host isn't numeric — see [`spec/stdlib.md`](https://github.com/aperio-lang/aperio/blob/main/spec/stdlib.md) C6 row), `accept_one(listen_fd) -> Int fallible(IoError)`, `recv_into(fd, buf, max_bytes) -> Int` (added 2026-05-19; `buf` is a `std::bytes::builder_new()` handle, the recv writes into the builder's tail with zero arena allocation — POSIX read(2) semantics: > 0 bytes appended, 0 peer closed, < 0 error), `close_fd(fd)` (infallible) |
| `std::io::udp` | `bind(host, port) -> Int fallible(IoError)` (SOCK_DGRAM; host="" → INADDR_ANY), `send(fd, host, port, msg) -> () fallible(IoError)`, `recv(fd, max_bytes) -> Bytes fallible(IoError)`, `recv_into(fd, buf, max_bytes) -> Int` (added 2026-05-19; same shape as tcp `recv_into`, single datagram per call, zero arena allocation), `close(fd)` (infallible). Datagram boundaries preserved by the kernel. **Not a bus transport**: UDP doesn't satisfy atomic delivery; cross-host bus over UDP needs a user adapter layering retry / framing on top. |
| `std::io::tls` | Client-side TLS via system OpenSSL (`-lssl -lcrypto`). `connect(host, port) -> Int fallible(IoError)` opens a TCP connection, performs a TLS 1.2+ handshake with SNI + system-trust-store cert verification, returns an opaque handle. `send_bytes(handle, b: Bytes) -> Int` and `recv_bytes(handle, max: Int) -> Bytes` operate on the handshaked connection (return 0/-1 and empty Bytes on error respectively, mirroring `std::io::tcp`'s `send_bytes`/`recv_bytes` shape). `recv_into(handle, buf, max_bytes) -> Int` (added 2026-05-19) reads directly into a `std::bytes::builder_new()` handle with zero arena allocation — same POSIX read(2) semantics as the tcp/udp siblings. `close(handle) -> Int` shuts the TLS layer down and closes the socket. |
