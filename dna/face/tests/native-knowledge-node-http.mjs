...
async function unchangedRefusal(command, code) {
  ...
  return { code, request_id: command.request_id };
}
...
try {
  ...
  await check('the session slice offers node proposal and its recovery', async () => {
    const slice = await service.slice();
    for (const name of ['KnowledgeNodePropose', 'KnowledgeNodeRevise', 'KnowledgeNodeRetire', 'KnowledgeLookup']) assert(slice.includes(name), name + ' in ' + JSON.stringify(slice));
    return { slice };
  });
  await check('8193 decoded text bytes are refused without admission', async () => {
    ...
    return unchangedRefusal(oversized, 'invalid_command');
  });
  await check('ungranted author and target scopes cannot create a node', async () => {
    ...
    for (const pair of [{ author: 'org/restricted', target: 'org' }, { author: 'org', target: 'org/restricted' }]) {
      ...
      values.push(await unchangedRefusal(forbidden, 'forbidden'));
    }
  ...
  await check('8192 control-heavy UTF8 bytes create an exact canonical candidate and Review', async () => {
    ...
    // the node line keeps the larger budget the forwarder gives it
    const encoded = bytes(JSON.stringify(wireLine(command))); assert(encoded > 32768 && encoded < 98304);
  ...
  await check('adopted control-heavy node is a valid exact relationship endpoint', async () => {
    ...
    const response = await service.post(edge); assert.equal(response.code, '', JSON.stringify(response));
    assert.equal(response.receipt.state, 'recorded'); assert.equal(response.receipt.operation, 'dna.knowledge.edge.link');
    const graph = await service.read(service.apiPath + '/dna/knowledge/edges?' + new URLSearchParams({ id: args.from_id }));
    assert.equal(graph.status, 200, JSON.stringify(graph));
    const stored = graph.json.data.items.find(row => row.id === response.receipt.edge_id);
    ...
    await save('edge-receipt.json', response.reply); await save('edge-read.json', graph.json);
  ...
  await check('node and edge operations share conflict-safe request identity', async () => {
    ...
    assert.equal(response.code, 'request_conflict', JSON.stringify(response));
    assert.equal(service.journal().head, before); assert.equal(admissions(command.request_id).length, 1);
    const found = await service.lookup(command.request_id); assert.equal(found.code, '', JSON.stringify(found));
    assert.equal(found.receipt.operation, 'dna.knowledge.node.propose'); assert.equal(found.receipt.command_id, adopted.command_id);
    return { request_id: command.request_id, code: response.code };
  });
} catch (error) {
