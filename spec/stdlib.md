...
| Fixed-size array bus-payload fields (`[T; N]`) | 2026-05-20 (Form H) | m70's wire codec previously rejected any array-typed payload field. This commit carves out the fixed-size case: a payload field declared `[T; N]` where T is a primitive (Int / Float / Decimal / Bool / Duration / Time) or a TypeRef (nested user-struct whose leaves are supported) now serializes / deserializes through `emit_array_field_serialize` / `emit_array_field_deserialize`. Primitive-element arrays take a single-memcpy fast path (`N * sizeof(elem)` bytes); TypeRef-element arrays loop N times, walking each nested struct inline. Arrays of String / Bytes and arrays-of-arrays remain deferred — the friction's "fixed-size case" framing. Closes the fixed-cap array-of-struct shape ask (`[Cell; 10]` where Cell is a flat record of scalars). Field storage and indexed read/write already worked pre-Form-H; only the wire codec was missing. |
| `BytesBuilder.append_slice` OOB closure split | 2026-05-20 | The C primitive's return contract now distinguishes 1=ok / 0=alloc-fail / -1=out-of-range (was 1=ok / 0=any-fail). The Aperio-side `append_slice` method branches on the three states and violates either the existing `alloc_failed` closure or the new `index_oob` closure (`captures: lo, hi`). Pre-split both routed through `alloc_failed`, which misled downstream production on_failure handlers — they read `captures.initial_cap` and concluded "memory exhausted" when the real cause was a caller-supplied bad index. Closes the append_slice routing friction. |
| View coercion at user-fn fallible + method-call arg sites; String/Bytes literal → View at storage defaults | F.30b follow-ons (5a, 5b) — 2026-05-20 | F.30 shipped the `view_coerces_to` predicate but only consulted it at non-fallible free-fn arg sites. **5a** extends the consultation to user-defined fallible-fn arg sites (the `fallible(E)` shape pond/websocket's `peek_header` uses), self-method + external-method + interface-dispatch + fn-pointer + monomorphized-generic arg sites. The unpack-and-check helpers from F.30b fire at every consultation. **5b** adds a literal-only coercion at storage-site defaults: a `String` / `Bytes` literal initializer for a `StringView` / `BytesView` field is wrapped via `lotus_view_from_static_data` with the static-epoch sentinel (the unpack helper sees the sentinel and returns `src` directly, no epoch check; the literal lives in the global string table at program-lifetime). Non-literal expressions still reject — those values might not have program-lifetime and shouldn't bypass the F.30 owned-vs-view distinction. Closes the StringView-default-init shape ask. |
| BytesView / StringView mutation-while-live runtime guard | F.30b (2026-05-20; ABI compacted 2026-05-22 PM) | `lotus_bytes_builder_t` gains a monotonic `mutation_epoch` field bumped by every mutating op (`append` / `append_slice` / `shift_front` / `clear` / `advance`). `view()` and `text_view()` return a 16-byte by-value struct `lotus_view_t { src: ptr, epoch: i64 }` — `src` is the builder pointer (or the static data pointer when `epoch == LOTUS_VIEW_EPOCH_STATIC = -1`); the data pointer is recomputed at unpack time. Read-site coercions (`view_coerces_to(BytesView, Bytes)` / `(StringView, String)`, the `len(view)` builtin, the println view arms) emit a call to `lotus_bytes_view_data` / `lotus_str_view_data` which compares the stamped epoch against the live epoch; mismatch → `lotus_view_stale_panic` (stderr diagnostic + `_exit(1)`). Catches "future maintainer added `b.append(...)` between `b.view()` and the consumer's read" loudly at the read site. Not closure-routed: stale-view is a programmer error, not a recoverable runtime condition — same shape as out-of-bounds array reads. Does NOT catch view-held-across-finish/dissolve (separate UAF concern; same shape as today's lifetime contract). 2026-05-22 PM compaction: was a 24-byte heap-allocated struct; now a by-value struct that lands in `{rax, rdx}` per SysV AMD64, eliminating the per-view arena allocation. |
...
- **Batteries included.** Go's approach: if a typical Aperio
  ...
  need third-party packages for specialized coordinator-
  system work.
...
| `std::time` | `monotonic() -> Duration`, `monotonic_ns() -> Int`, `sleep(d: Duration)`, `now() -> Int`, `time_from_unix(n: Int) -> Time` | `clock_gettime(CLOCK_MONOTONIC)` + EINTR-retrying `clock_nanosleep`; `monotonic_ns` returns the same `clock_gettime` reading typed as `Int` (i64 ns) — skips the `Duration`-to-`Int` ASCII round-trip downstream hot paths were doing (`to_string(monotonic())` → strip `"ns"` → `parse_int`); `now()` calls `clock_gettime(CLOCK_REALTIME)` via `lotus_time_now_seconds`; `time_from_unix` formats `gmtime_r` + `strftime` ISO 8601 UTC via `lotus_time_from_unix` |
| `std::decimal` | `to_float(d: Decimal) -> Float` | 2026-05-21. Direct i128 → f64 conversion at scale 9 (`mantissa × 10^-9`) via `lotus_decimal_to_float`. Replaces the `to_string(d)` → strip → `parse_float` ASCII round-trip downstream hot paths were doing for Decimal → Float (e.g. metrics gauges setting Float values from Decimal prices). Same fix shape as `monotonic_ns`. |
...
ship now. Pair with a daemon's observability shape: stamp RSS
into a metrics gauge once per heartbeat tick, alarm on growth
slopes.
...
line rate (one workload logged 1.6M lines in seconds). The
