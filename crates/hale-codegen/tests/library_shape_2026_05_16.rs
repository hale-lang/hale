...
fn or_discard_swallows_unit_error() {
    let src = r#"
        fn main() {
            std::io::fs::write_file("/tmp/hale_test_discard.txt", "ok") or discard;
            std::io::fs::mkdir("/tmp/hale_test_discard_dir") or discard;
            println("ok");
        }
    "#;
    let (stdout, status) = build_and_run("discard_unit", src, &[]);
    let _ = std::fs::remove_file("/tmp/hale_test_discard.txt");
    let _ = std::fs::remove_dir_all("/tmp/hale_test_discard_dir");
    assert!(status.success(), "non-zero: {:?}", status);
