...
(`dna.unix.member`, in the record's local config). A leg is not asked
who it is — there is no holder flag — and a peer whose person holds
no position sees no verb at all: the leg says so at attach, before it
asks anything. `hale describe <socket>` lists what a peer may call;
