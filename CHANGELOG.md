...
- **`hale inputs <seed>`** is new: every file a build of the seed reads, one canonical path per line, resolved as the compiler resolves imports. For anything that has to know what a build depends on without guessing from git.
