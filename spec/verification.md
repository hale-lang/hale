...
| **`ring_layout` contract** | a foreign-ring layout declaration that's internally ill-formed — unknown scalar/`len_prefix` repr, missing `framing` (or `byte_records` without a `len_prefix`), no cursor / a cursor without an `at`, unknown cursor ordering or unit, negative offsets — or a `shm_ring(..., layout: N)` whose `N` doesn't resolve to a declared `ring_layout` | error | `check_ring_layout` + `check_main_and_bindings` |
| **`ring_layout` geometry** | a *cross-field* inconsistency in a layout that would silently corrupt the reader: a header scalar or the cursor overrunning `data_at`, two header fields overlapping, a non-power-of-two record `align`, or a `pad_sentinel` too wide for the `len_prefix` to hold | error | `check_ring_layout` |
| **Foreign-ring payload shape** | a `layout:`-bound topic whose payload isn't flat-shapeable — a foreign record is read by direct pointer-cast and written by `memcpy`, so it needs a fixed byte layout (enforced regardless of `where zero_copy`) | error | `check_main_and_bindings` |
