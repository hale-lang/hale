...
- **Supersession is the Board's decision.** A proposal that supersedes an earlier digest retires it when ratified (`knowledge.retired`, appended by the assembly) and leaves it in force when declined; `upgrade` proposes each practice whose text changed, superseding the version in the record, and nothing for the rest. Both stores retire an idea by marking it not accepted: it leaves every package and stays readable by digest.
