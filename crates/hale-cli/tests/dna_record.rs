...
fn the_record_is_a_branch_every_clone_can_fetch() {
    ...
    assert_eq!(count, "35", "one commit per seed event");
    ...
    assert!(ok && st.contains("35 event(s), chain verified"), "{st}");
    ...
    assert!(ok && after.contains("35 event(s), chain verified") && after.contains("review.requested"), "{after}");
