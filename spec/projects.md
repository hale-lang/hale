...
| `--target native\|wasm32\|<triple>` | The target to build for; `hale --list-targets` names every triple with its tier as seen from this host. `native` (default) and the host's own triple build and link the host platform; `wasm32` builds from any host. Another host's native triple (`--target x86_64-unknown-linux-gnu` on a Mac) is a cross target: the build emits a relocatable object for it — the target's own backend, triple and a generic CPU, named `<stem>.o` — and stops there with a note, since linking needs the lotus runtime and system libraries built for the target (GH #970; before it, GH #969, such a triple silently built the host). `run` and `test` refuse it, as they refuse `wasm32`. A Windows triple is refused at argument parsing as not buildable yet (GH #445). The check judges target questions — `where async_io` — against the named target, not the host. |
