...
| `assembly.hl` (card 16) | `Dna.on_case_requested` / `hand_case` / `ensure_case_rows` / `restore_cases`: a human attempt admitted as its own case `<task>.s<i>.<key>` (`case_id_of`), the versioned `case.admitted` exact at its reading before `task.born` / `task.handed`; `handoff_terms` binds assignee, obligation, acceptance and required evidence as a job's (the leader's word through `on_planned` where the organism plans); `case_of_attempt`, `case_admitted` (never edit recovery); `held_cases` for `redrive` |
