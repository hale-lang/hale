...
| delivery across off-thread bindings: the membrane's intent reaches `Dna.ask` over the organism's bound socket and the admission lands (`dna_run.rs`); from there every transition is a resident's own, so no step of an execution waits on a delivery across a binding (F.15 closed) | proven, cards 18–19 |
