...
fn init(app_dir: &Path) -> Result<Vec<String>, String> {
    ...
    created(&mut out, &org_dir.join("work.hl"), work_hl())?;
...
fn upgrade(dir: &Path) -> Result<Vec<String>, String> {
    ...
    if org_dir.join("main.hl").is_file() && !performers.is_file() {
        fs::write(&performers, work_hl()).map_err(|e| format!("write {}: {e}", performers.display()))?;
...
/// model performer arrives with the model leg.
fn work_hl() -> &'static str {
    r#"// dna/org/work.hl — the performers (project-owned; generated by
// `hale dna init`).
//
...
// for the work kinds it takes), and a model (the model leg, when it
// lands). `hale check` validates all of it.
...
// for them. `legs::NoDeterministic { }` takes nothing.
fn deterministic() -> legs::NoDeterministic {
    return legs::NoDeterministic { };
}

fn performers() -> legs::PerformerCatalog {
    return legs::PerformerCatalog { person: legs::Person { }, deterministic: deterministic(), model: legs::NoModel { } };
}
