...
| `BytesView` (F.30, 2026-05-20) | ptr → `[i64 len][u8 data[len]]` (same runtime layout as `Bytes`) | Non-owning view over a `BytesBuilder`'s buffer. Returned by `BytesBuilder.view()`. Coerces implicitly to `Bytes` at function-argument READ positions (e.g. `std::bytes::at(view, i)`, `len(view)`) — same pointer ABI, no runtime cost. Rejected at storage sites whose declared type is `Bytes` (let-ascription with `let x: Bytes = ...`, struct-field-init, locus-param-init); callers wanting owned storage must explicitly call `std::bytes::clone(view)` for a deep-copy into the caller's arena. Storage typed `BytesView` is allowed and signals the non-owning intent in the type signature. Lifetime is documented-and-trusted at v1: valid until the next mutation on the source builder. The mutation-while-view-live diagnostic is deferred to F.30b (runtime epoch guard or v2 borrow checker). |
| `StringView` (F.30, 2026-05-20) | ptr (NUL-terminated, same runtime layout as `String`) | Non-owning view over a `BytesBuilder`'s NUL-terminated buffer. Returned by `BytesBuilder.text_view()`. Symmetric companion to `BytesView`: coerces to `String` at read sites (`len`, `println`, `std::str::*`); rejected at `String`-typed storage; `std::str::clone(view)` upgrades to owned. Same lifetime contract as `BytesView`. |
