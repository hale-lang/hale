...
# sessions pinned to bin/hale. Never publish without first
...
#                   tests itself.
#   --skip-build    Reuse target/release/hale instead of
...
A publish to bin/hale affects every app session pinned to it.
...
SRC="$ROOT/target/release/hale"
DST="$ROOT/bin/hale"

# Tree-sitter staticlib produced by the `hale-ts-shim` workspace
# crate. The hale binary links against this at user-program
...
if [[ ! -f "$TS_SHIM_SRC" ]]; then
    ...
    echo "side-effect of building hale-ts-shim)." >&2
    exit 1
fi
...
hale published-stable snapshot
commit: $GIT_SHA
branch: $GIT_BRANCH
date:   $DATE
artifacts:
  - bin/hale  (compiler binary)
  - bin/libhale_ts_shim.a  (tree-sitter shim — link dependency
