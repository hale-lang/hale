...
`run` takes `build`'s options as well — `--dev`, `--target-cpu`,
`--link`, `--csrc` — and builds what `build` would build with them, so
`hale run --csrc glue.c app.hl` runs an `@ffi` program. They are part
