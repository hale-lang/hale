...
### dna tests: a fixture takes a port it has bound (GH #873)
...
- `dna/tests/support_test.hl` is the fixtures' own support under test: three indices answer three ports in range, and with the port index 0 named held by this fixture, index 0 must step past it. Against the old arithmetic that last assertion fails, naming the port handed out twice. No organism is scaffolded or booted, so it costs its slice under a second.
