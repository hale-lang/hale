...
fn pub_src(sock: &str) -> String {
    format!(
        ...
        main locus App {{
            ...
            run() {{
                // Settle before the first send. GH #468 made the
                // realize-to-registration window lossless (the
                // reader buffers + flushes at registration), so
                // this sleep is no longer load-bearing for LOSS —
                // it is kept so the recorded artifact's delivery
                // ORDER is boringly deterministic for the replay
