...
fn parse_and_run(rel: &str) -> Result<i32, String> {
    ...
    let program = aperio_syntax::parse_source(&source)
        .map_err(|d| {
            d.iter()
...
#[test]
fn hello_world_runs() {
    assert_eq!(parse_and_run("hello-world/main.ap").unwrap(), 0);
}
...
fn locus_with_run_runs() {
    ...
    assert_eq!(parse_and_run("01-locus-with-run/main.ap").unwrap(), 0);
}

#[test]
fn parent_child_runs() {
    assert_eq!(parse_and_run("02-parent-child/main.ap").unwrap(), 0);
}

#[test]
fn closure_test_runs() {
    assert_eq!(parse_and_run("03-closure-test/main.ap").unwrap(), 0);
}

#[test]
fn closure_absorbed_runs() {
    assert_eq!(parse_and_run("03b-closure-absorbed/main.ap").unwrap(), 0);
}
...
fn closure_bubbled_exits_nonzero() {
    ...
    let result = parse_and_run("03c-closure-bubbled/main.ap");
...
#[test]
fn modes_runs() {
    assert_eq!(parse_and_run("04-modes/main.ap").unwrap(), 0);
}

#[test]
fn mutable_counter_runs() {
    assert_eq!(
        parse_and_run("06-mutable-counter/main.ap").unwrap(),
        0
    );
}
...
fn control_flow_runs() {
    assert_eq!(
        parse_and_run("07-control-flow/main.ap").unwrap(),
        0
    );
}
...
fn monotonic_sleep_runs() {
    ...
    assert_eq!(
        parse_and_run("08-monotonic-sleep/main.ap").unwrap(),
        0
    );
}
...
fn functions_runs() {
    assert_eq!(
        parse_and_run("09-functions/main.ap").unwrap(),
        0
    );
}
...
fn stateful_locus_runs() {
    assert_eq!(
        parse_and_run("10-stateful-locus/main.ap").unwrap(),
        0
    );
}
...
#[test]
fn bus_runs() {
    assert_eq!(parse_and_run("05-bus/main.ap").unwrap(), 0);
}
...
fn bus_runs_under_ringbuffer_transport() {
    ...
    let program = aperio_syntax::parse_source(&source).unwrap();
