//! Form K4c (2026-05-20) — end-to-end test: an Hale publisher
...
#[test]
fn hale_publisher_routes_through_shm_ring() {
    let tag = unique_tag("pub");
    let shm_name = format!("/hale-{}", tag);
    let n_pub: i64 = 5;
    let slot_count: u64 = 8;

    // Hale publisher source.
    //
...
///
/// Builds a publisher-only Hale binary, runs it to completion,
