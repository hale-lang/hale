...
pub fn run_bundle(programs: &[&Program]) -> Result<i32, String> {
    ...
    let owned: Vec<Program> = programs
        ...
        .map(|p| {
            ...
            hale_syntax::desugar::desugar_intra_locus_topics(&mut clone);
            hale_syntax::desugar::desugar_topics(&mut clone);
            clone
        })
        .collect();
    for p in &owned {
...
impl Interpreter {
        ...
        match stmt {
            ...
            Stmt::Violate { name, payload, span: _ } => {
                ...
                let closure_decl = handle
                    ...
                    .iter()
                    .find_map(|m| match m {
                        hale_syntax::ast::LocusMember::Closure(c)
                ...
                let is_inline = closure_decl.clauses.iter().any(|c| matches!(
                    c,
                    hale_syntax::ast::ClosureClause::Epoch(
                        hale_syntax::ast::EpochSpec::Inline
                    )
                ));
                if !is_inline {
                    return Err(Signal::Error(format!(
                ...
                let captures: Vec<String> = closure_decl
                    ...
                    .iter()
                    .flat_map(|c| match c {
                        hale_syntax::ast::ClosureClause::Captures(names) => {
    ...
    fn find_hashmap_counter_field(
        ...
        use hale_syntax::ast::{LocusMember, PrimType, TypeExpr};
    ...
    fn build_bump_initial_struct(
        ...
        use hale_syntax::ast::{LocusMember, TypeExpr};
        let cell_type_name = handle
            .decl
            .members
            .iter()
