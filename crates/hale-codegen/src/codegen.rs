...
///   2. Sibling of the running hale binary
///      (`$(dirname current_exe)/libhale_ts_shim.a`) — the
///      `bin/hale` publish ships the staticlib alongside the
///      binary so app teams pinned to `bin/hale` link cleanly
...
fn locate_ts_shim_staticlib() -> Option<PathBuf> {
    ...
    if let Ok(exe) = std::env::current_exe() {
        if let Some(dir) = exe.parent() {
            let p = dir.join("libhale_ts_shim.a");
            if p.exists() {
                return Some(p);
            }
        }
    }
    // CARGO_MANIFEST_DIR at build time of hale-codegen is
    // `<workspace>/crates/hale-codegen`. Workspace root is two
    // dirs up.
...
pub(crate) struct Cx<'ctx, 'p> {
    ...
    pub(crate) current_user_fn_ret: Option<Option<CodegenTy>>,
    ...
    pub(crate) current_self: Option<SelfCx<'ctx>>,
    ...
    pub(crate) user_fns: BTreeMap<String, FnSig<'ctx>>,
    ...
    pub(crate) user_types: BTreeMap<String, TypeInfo<'ctx>>,
    ...
    pub(crate) user_interfaces: BTreeSet<String>,
    ...
    pub(crate) bus_state: Option<BusState>,
    ...
    pub(crate) shm_ring_subjects: std::collections::BTreeMap<String, ShmRingBindingInfo>,
    ...
    /// loop completes.
    pub(crate) current_arena_override: Option<PointerValue<'ctx>>,
    ...
    pub(crate) current_user_fn_exit_bb: Option<inkwell::basic_block::BasicBlock<'ctx>>,
    pub(crate) current_user_fn_ret_alloca: Option<PointerValue<'ctx>>,
    ...
    pub(crate) current_method_scratch: Option<PointerValue<'ctx>>,
    ...
    /// us).
    pub(crate) current_method_caller_arena: Option<PointerValue<'ctx>>,
    ...
    pub(crate) current_user_fn_fallible: Option<FallibleCtx<'ctx>>,
    ...
    /// locus instantiated as a field of that outer locus must
    /// also live in the payload arena: otherwise the outer
    /// struct holds dangling stack-alloca pointers to its
    /// children after the fn returns. Pre-fix, only the OUTER
    /// locus took the payload-arena path; children landed in
    /// the fn's stack via `alloca_in_entry_with_nulled_arena`.
    /// Some children's stale stack happened to read back the
    /// right bytes (because the post-return stack hadn't been
    /// overwritten yet); others corrupted to garbage. The
    /// "Mapper with N >= 3 @form-hashmap children returned
    /// through fallible context" repro pinned this.
    ///
    /// `mem::take` discipline: outermost m90-routed
    /// instantiation owns the flag, children consume it in
    /// their own routing block (forcing their self_ptr to
    /// payload arena), and a fresh child instantiation under
    ...
    /// `resolve_failure_route` to find the parent's self_ptr +
    /// on_failure handler — pre-F.31 the parent context came
    /// from `current_self` (set by lifecycle method-body
    /// lowering), but placement-pinned children instantiated
    /// as main-locus params defaults run BEFORE any method
    /// body, so `current_self` is None and the routing would
    /// resolve null. This field is the fallback. Cleared after
    /// the params-init loop completes; outer
    ...
    /// (via `mem::take`) right after the placement override and
    /// stores it on `current_cooperative_pool` for the duration
    /// of this instantiation. Used by `emit_bus_register` to
    /// route the subscriber's registration through
    /// `lotus_bus_register_with_pool` with the right pool ptr.
    pub(crate) cooperative_pool_for_next_locus_instantiation: Option<String>,
    /// F.31 Phase 4: the current cooperative-pool name (if any)
    /// in effect during a locus instantiation. Set at the top
    /// of `lower_locus_instantiation` from the consumed
    /// `cooperative_pool_for_next_locus_instantiation`, restored
...
#[derive(Debug, Clone, Copy)]
pub(crate) struct SerializerPair<'ctx> {
    ...
    /// wire-format pass.
    pub(crate) serialize: FunctionValue<'ctx>,
...
#[derive(Debug, Clone)]
pub(crate) struct AccumulatorCtx<'ctx> {
    pub(crate) slots: Vec<AccumulatorSlot>,
    pub(crate) next_idx: usize,
    pub(crate) self_ptr: PointerValue<'ctx>,
    pub(crate) struct_ty: inkwell::types::StructType<'ctx>,
}
...
pub(crate) struct RoutingKeySubjectInfo {
    ...
    pub(crate) keyed_by_field: String,
    ...
    pub(crate) policy: Option<hale_syntax::ast::UnmatchedPolicy>,
}

#[derive(Debug, Clone)]
#[allow(dead_code)]  // K7's `overflow` reserved for K7b fallible-`<-`
pub(crate) struct ShmRingBindingInfo {
...
pub(crate) struct EnumInfo {
    pub(crate) variants: Vec<EnumVariantInfo>,
...
pub(crate) struct EnumVariantInfo {
    ...
    pub(crate) field_tys: Vec<CodegenTy>,
}
...
#[derive(Debug, Clone)]
pub(crate) struct AccumulatorSlot {
    pub(crate) kind: AccumulatorKind,
    pub(crate) inner_expr: Option<Expr>,
    pub(crate) ty: CodegenTy,
    pub(crate) inner_ty: CodegenTy,
    pub(crate) field_idx: u32,
    pub(crate) field_idx_2: Option<u32>,
}

#[derive(Debug, Clone)]
pub(crate) struct FnSig<'ctx> {
    pub(crate) func: FunctionValue<'ctx>,
    pub(crate) params: Vec<CodegenTy>,
    ...
    /// `None` = void (no return type in the Hale declaration).
    pub(crate) ret: Option<CodegenTy>,
    ...
    pub(crate) fallible: Option<CodegenTy>,
    ...
    /// no fallible sret slots, no Hale-internal type wrapping).
...
pub(crate) fn view_coerces_to(from: &CodegenTy, to: &CodegenTy) -> bool {
    matches!(
        (from, to),
...
pub(crate) fn literal_to_view_coerces(
    from: &CodegenTy,
    to: &CodegenTy,
    expr: &Expr,
) -> bool {
...
pub(crate) fn count_self_field_accesses_in_locus(l: &LocusDecl) -> std::collections::BTreeMap<String, u32> {
...
/// that pool.
pub(crate) fn chunk_hint_for_coop_pool(
...
pub(crate) fn locus_reads_self_children(l: &LocusDecl) -> bool {
...
pub(crate) struct FallibleCallResult<'ctx> {
    pub(crate) i1_path: IntValue<'ctx>,
    pub(crate) out_val_slot: Option<PointerValue<'ctx>>,
    pub(crate) out_err_slot: PointerValue<'ctx>,
    pub(crate) success_ty: Option<CodegenTy>,
    pub(crate) payload_ty: CodegenTy,
}

/// v1.x-FORM-2 PR6: per-call fallible-fn lowering state. See
/// `current_user_fn_fallible` for usage.
#[derive(Debug, Clone)]
pub(crate) struct FallibleCtx<'ctx> {
    /// Local stage for the error payload. `Stmt::Fail` lowers
    /// into here; the exit epilogue sret-copies into
    /// `out_err_param`.
    pub(crate) err_alloca: PointerValue<'ctx>,
    ...
    pub(crate) path_alloca: PointerValue<'ctx>,
    ...
    pub(crate) out_val_param: Option<PointerValue<'ctx>>,
    /// Caller-provided sret slot for the error payload.
    pub(crate) out_err_param: PointerValue<'ctx>,
...
#[derive(Debug, Clone)]
pub(crate) struct LocusInfo<'ctx> {
    pub(crate) struct_ty: StructType<'ctx>,
    /// Field name → (index in struct, field type).
    pub(crate) fields: BTreeMap<String, (u32, CodegenTy)>,
    ...
    pub(crate) defaults: Vec<(String, DefaultInit)>,
    ...
    pub(crate) methods: BTreeMap<&'static str, FunctionValue<'ctx>>,
    ...
    pub(crate) user_methods: BTreeMap<String, FunctionValue<'ctx>>,
    ...
    /// by epoch.
    pub(crate) closures: Vec<(String, ClosureAssertion, EpochSpec)>,
    ...
    pub(crate) birth_closures_fn: Option<FunctionValue<'ctx>>,
    ...
    /// an unambiguous slot.)
    pub(crate) dissolve_closures_fn: Option<FunctionValue<'ctx>>,
    ...
    pub(crate) tick_closures_fn: Option<FunctionValue<'ctx>>,
    ...
    pub(crate) quarantined_field_idx: u32,
    ...
    pub(crate) restart_in_place_pending_field_idx: u32,
    ...
    pub(crate) slot_borrowed_mask_field_idx: u32,
    ...
    pub(crate) recpool_field_idx: u32,
    ...
    pub(crate) recpool_release_pool_field_idx: u32,
    ...
    pub(crate) recpool_release_kind_field_idx: u32,
    ...
    pub(crate) parent_on_failure_field_idx: u32,
    ...
    /// when so.
    pub(crate) duration_last_fire_field_idxs: Vec<u32>,
    ...
    pub(crate) duration_closures_fn: Option<FunctionValue<'ctx>>,
    ...
    pub(crate) duration_wrapper_fn: Option<FunctionValue<'ctx>>,
    ...
    pub(crate) explicit_closures_fn: Option<FunctionValue<'ctx>>,
    ...
    pub(crate) projection_class: ProjectionClass,
    ...
    #[allow(dead_code)]
    pub(crate) schedule_class: ScheduleClass,
    ...
    pub(crate) capacity_slots: Vec<CapacitySlotLayout>,
...
#[derive(Debug, Clone)]
pub(crate) struct CapacitySlotLayout {
    pub(crate) name: String,
    pub(crate) kind: CapacitySlotKind,
    ...
    pub(crate) as_parent_for: Option<String>,
    ...
    pub(crate) struct_field_idx: u32,
    ...
    pub(crate) form: Option<SlotForm>,
    ...
    pub(crate) indexed_by: Option<String>,
    ...
    pub(crate) ring_buffer_cap: Option<u64>,
...
pub(crate) struct TypeInfo<'ctx> {
    ...
    pub(crate) defaults: BTreeMap<String, Expr>,
}
...
#[derive(Debug, Clone)]
pub(crate) struct SelfCx<'ctx> {
    pub(crate) locus_name: String,
    pub(crate) struct_ty: StructType<'ctx>,
    pub(crate) self_ptr: PointerValue<'ctx>,
    pub(crate) fields: BTreeMap<String, (u32, CodegenTy)>,
}
...
impl<'ctx, 'p> Cx<'ctx, 'p> {
    ...
    pub(crate) fn flush_dissolve_frame_kind(
        &mut self,
        drain_queue: bool,
    ) -> Result<(), CodegenError> {
    ...
    /// `<L>.run(self_ptr)` and returns. Used at instantiation
    /// sites for non-main-cooperative-pool main-locus params
    /// fields: the synchronous `run()` call is replaced with a
    /// `lotus_coop_pool_post(pool_ptr, wrapper, self, NULL, 0)`
    /// so the body executes on the pool's worker thread.
    ///
    /// Saves and restores the builder position so this can run
    /// in the Pass A3 zone without disturbing later passes.
    fn synthesize_coop_pool_run_wrappers(&mut self) -> Result<(), CodegenError> {
        let saved_block = self.builder.get_insert_block();
        let void_t = self.context.void_type();
        let ptr_t = self.context.ptr_type(AddressSpace::default());
        ...
        for locus_name in &types {
            ...
            let run_fn = match info.methods.get("run") {
                Some(f) => *f,
                None => continue,
            };
            let wrapper_name = format!("__coop_pool_run_{}", locus_name);
            let wrapper_ty =
    ...
    pub(crate) fn emit_str_len_call(
        &self,
        s: PointerValue<'ctx>,
    ...
    pub(crate) fn key_value_to_i64_pair(
        &self,
    ...
    fn lower_program(&mut self) -> Result<(), CodegenError> {
        ...
        // F.31 Phase 4b: synthesize `__coop_pool_run_<L>(self,
        // payload)` wrappers for locus types that appear as
        // non-main cooperative-pool main-locus params fields.
        // The wrapper's signature matches the pool-handler ABI
        // `void(ptr, ptr)`; its body discards the payload arg
        // and calls the locus's own `run` method. Instantiation
        // sites for these loci skip the synchronous `run()` call
        // and instead post the wrapper to the pool's queue, so
        ...
        // F.31 Phase 4: register each distinct cooperative pool
        // referenced in `placement { }`, then spawn one worker
        // pthread per pool. Pools must be registered BEFORE any
        // locus instantiation so bus_register's coop_pool ptr
        // lookup resolves to a non-null entry. start_all must
        ...
        if !distinct_pools.is_empty() {
            ...
            let start_all_fn = self
                .module
                .get_function("lotus_coop_pool_start_all")
                .expect("lotus_coop_pool_start_all declared");
            self.builder
                .build_call(start_all_fn, &[], "coop_pool.start_all")
                .map_err(|e| CodegenError::LlvmEmit(e.to_string()))?;
        }
    ...
    /// (computed by `hale-types::resolve` and propagated via
    ...
    fn declare_user_fn(&mut self, f: &FnDecl) -> Result<(), CodegenError> {
        ...
        // prefix, no fallible sret slots, no Hale-internal type
        // wrapping. The LLVM function is `add_function`-emitted
        // with the literal Hale fn name as the C symbol; with
        ...
        // v1.x-FORM-2 PR6: fallible(E) lowering. The Hale-level
    ...
    /// LLVM level. The C-ABI signature is built from the Hale
    /// types per `spec/ffi.md`'s marshalling table — no implicit
    /// `__caller_arena` prefix, no fallible sret slots, no
    /// monomorphization. The LLVM symbol is the literal Hale fn
    ...
    fn declare_ffi_fn(&mut self, f: &FnDecl) -> Result<(), CodegenError> {
        ...
        // sret-style struct returns: when the Hale fn declares
        // `-> TypeRef(...)`, the LLVM signature gets a hidden ptr
        // param prepended (caller allocates the slot, callee
        // fills) and returns void. The Hale call site allocates
    ...
    /// FFI parameter-type lowering. Maps an Hale codegen type to
    ...
    fn llvm_ffi_param_type(
        ...
        match ty {
            ...
            CodegenTy::TypeRef(name) => {
                ...
                // passed by POINTER, not by value. The Hale side
    ...
    fn lower_user_fn_body(&mut self, f: &FnDecl) -> Result<(), CodegenError> {
        ...
        if f.ffi.is_some() {
            // Stage-1 FFI: @ffi fns have no Hale-side body to
    ...
    /// A downstream MetricMap residency dump pinned this:
    ...
    /// is the dominant win.
    pub(crate) fn emit_cross_arena_store_deep_copy(
        &mut self,
        ...
        for (i, arg) in args.iter().enumerate() {
            ...
            let coerced: inkwell::values::BasicValueEnum<'ctx> =
                match expected {
                    ...
                    CodegenTy::TypeRef(_) => {
                        // User-type struct passes by pointer. Hale
    ...
    fn lower_user_fn_call(
        ...
        for i in 0..sig.params.len() {
            ...
            // widens via sitofp (resolves notes/hale-friction.md
    ...
    pub(crate) fn lower_std_os_getrandom_fallible(
        &mut self,
        args: &[Expr],
        scope: &Scope<'ctx>,
    ...
    pub(crate) fn try_lower_user_locus_fallible_method(
        &mut self,
        info: &LocusInfo<'ctx>,
        self_ptr: PointerValue<'ctx>,
        locus_name: &str,
    ...
    pub(crate) fn emit_index_error_alloc(
        ...
        let info = self
            ...
            .get("IndexError")
            .cloned()
            .expect("IndexError injected by hale-types resolver");
        let size = info
            .struct_ty
            .size_of()
    ...
    pub(crate) fn emit_key_error_alloc(
        ...
        let info = self
            ...
            .get("KeyError")
            .cloned()
            .expect("KeyError injected by hale-types resolver");
        let size = info
            .struct_ty
            .size_of()
    ...
    pub(crate) fn build_alloc_bus_unmatched_key(
        &mut self,
    ...
    pub(crate) fn emit_empty_error_alloc(
        &mut self,
        kind: &str,
    ) -> Result<PointerValue<'ctx>, CodegenError> {
        let info = self
            ...
            .get("EmptyError")
            .cloned()
            .expect("EmptyError injected by hale-types resolver");
        let size = info
            .struct_ty
            .size_of()
    ...
    /// surface area today.
    pub(crate) fn payload_typename_global(&mut self, ty: &CodegenTy) -> PointerValue<'ctx> {
        let name = match ty {
    ...
    pub(crate) fn lower_stmt(
        ...
        match stmt {
            ...
            Stmt::Yield(_) => {
                ...
                self.emit_bus_drain()?;
                // 2026-05-23: also drain the
                // current thread's pinned mailbox (TLS-cached),
                // so a pinned locus's run() loop with explicit
                // `yield;` sees cells posted by cooperative
                // publishers mid-program. No-op on cooperative
                // threads (TLS is null).
    ...
    fn lower_return(
        ...
        match (expr, ret_ty) {
            ...
            (Some(e), Some(declared_ty)) => {
                ...
                // (2026-05-22 PM, a downstream daemon / SymbolBook
    ...
    pub(crate) fn alloca_for(
        ...
        // `stream_aggregator.hl`'s `let v = (i * 31 + 7) % 1000`
    ...
    pub(crate) fn lower_expr(
        ...
        match e {
                ...
                if matches!(receiver.as_ref(), Expr::KwSelf(_)) =>
            {
                ...
                // back to `params_init_self` — the parent locus
                // whose params are currently being initialized.
    ...
    /// representation).
    pub(crate) fn llvm_tuple_storage_type(
        &self,
        elems: &[CodegenTy],
    ...
    pub(crate) fn emit_pinned_mailbox_drain_pending(&mut self) -> Result<(), CodegenError> {
        let get_current_fn = self
            ...
            .expect("lotus_mailbox_get_current declared in declare_builtins");
        let mb_ptr = self
            .builder
            .build_call(get_current_fn, &[], "mailbox.get_current")
            .map_err(|e| CodegenError::LlvmEmit(e.to_string()))?
            .try_as_basic_value()
            .left()
            .expect("lotus_mailbox_get_current returns ptr")
            .into_pointer_value();
        let drain_pending_fn = self
            .module
            .get_function("lotus_mailbox_drain_pending")
            .expect("lotus_mailbox_drain_pending declared in declare_builtins");
        self.builder
            .build_call(
                drain_pending_fn,
                &[mb_ptr.into()],
    ...
    //
    // `.hl` source references stdlib symbols by fully-qualified
    ...
    /// position. Resolves notes/hale-friction.md 2026-05-10
        ...
        let buf_handle = match &buf_ty {
            CodegenTy::LocusRef(n) if n == "__StdBytesBytesBuilder" => {
                ...
                let handle_int = self
                    ...
                    .map_err(|e| CodegenError::LlvmEmit(e.to_string()))?;
                // `handle` is Int (i64) — see bytes_builder.hl. The
    ...
    pub(crate) fn emit_or_get_sort_trampoline(
        &mut self,
        elem_ty: &CodegenTy,
        reverse: bool,
    ...
    /// `persists_through`.
    pub(crate) fn zero_accumulator_slot(
        &mut self,
    ...
    pub(crate) fn update_accumulator_slot(
        &mut self,
        struct_ty: inkwell::types::StructType<'ctx>,
        self_ptr: PointerValue<'ctx>,
        slot: &AccumulatorSlot,
    ...
    pub(crate) fn lower_enum_variant_alloc(
        &mut self,
        info: &EnumInfo,
        tag: u32,
    ...
    pub(crate) fn load_enum_payload_fields(
        &mut self,
        info: &EnumInfo,
        enum_ptr: PointerValue<'ctx>,
        variant_idx: usize,
    ...
    pub(crate) fn compound_storage_size(
        &self,
        ty: &CodegenTy,
    ...
    pub(crate) fn open_method_scratch(&mut self) -> Result<(), CodegenError> {
        let cs = self
            .current_self
            .clone()
    ...
    pub(crate) fn emit_set_caller_arena(&mut self) -> Result<(), CodegenError> {
        let arena = self.current_arena_ptr()?;
        let set_fn = self
            .module
    ...
    pub(crate) fn current_arena_ptr(
        &mut self,
    ) -> Result<PointerValue<'ctx>, CodegenError> {
    ...
    pub(crate) fn alloca_in_entry_with_nulled_arena(
        &mut self,
        struct_ty: inkwell::types::StructType<'ctx>,
        arena_field_idx: u32,
        name: &str,
...
pub(crate) struct Scope<'ctx> {
    pub(crate) locals: BTreeMap<String, (PointerValue<'ctx>, CodegenTy)>,
}
...
pub(crate) fn param_value(e: &Expr) -> Result<ParamValue, CodegenError> {
    match e {
...
pub(crate) fn codegen_ty_size_bytes<'ctx>(
