...
This is the DNA prerelease. An organization written in Hale now has a workflow — fifteen durable-first cards, from a routed retry to an edit outcome — over three memories with owners, funding and leases, a record with an API of its own and the protections a business needs (retention, protected evidence, attributed decisions, a principal source), and a host that runs the body on a server and fences it with a lease. It is a prerelease because the shapes are still moving under fixtures rather than under customers; the DNA book describes what holds today.

Underneath it the compiler stopped letting `hale check` and `hale build` disagree, locus ownership is decided by a table before any code is emitted, and every command renders a diagnostic at the span the author wrote.

### DNA

**The workflow.** Fifteen cards, each one durable-first: nothing runs before the record holds the fact that it may.

- Card 01 — a routed retry carries its own attempt number: each try gets the same request with only `attempt_no` replaced, so the performer, the Attempt record, the history and an agent's model evidence name the attempt the record names.
- Card 02 — recovery finds a Task's Mutation in either memory's order: the Mutation is found by the Task it names anywhere in the journal, so a split record + ledger resumes exactly as one journal does instead of replanning a Task awaiting its Review.
- Card 04 — code-authored workflow definitions and their bound expansion: a catalog of plain definitions, and an expansion that binds one execution's whole finite tree with its Task, Step and Work ids or refuses it whole, counting against the admission limits before anything is materialized.
- Card 05 — the durable facts: versioned codecs for the seven facts one execution leaves behind, read whole or not at all, with every identity checked against the fact it belongs to (an absent revision is not revision 0, and absent limits are not zero limits).
- Card 06 — the state of an execution, read from its facts: one row is applied, replayed or refused, and applying reaches nothing outside the projection — its entry point carries `@no_syscall`, which the compiler checks through every method it reaches. The admitted recipe is the authority for every transition.
- Card 07 — an execution is admitted atomically: the assembly mints the id, expands the definition under its limits and appends the admission with exact compare-and-append before anything could run. A refusal is a row of its own under the minted id, and the ask's id is the admission's identity, so the same ask again is the same execution.
- Card 08 — exactly one admitted attempt: an attempt is claimed by an exact append at a captured revision before the performer is called, so an outcome recorded meanwhile is the answer rather than a second run, and a request arriving while it runs attaches to its pending outcome. Every reply is judged for whose it is before it is accepted.
- Card 09 — one step alive across delayed replies: resident loci for one active step and its leaf Works, hearing an immediate and a delayed reply through one reducer. A refusal *by the record* is no outcome — a Work whose append the record refused holds its transition, publishes no settlement it does not have, and proposes it again when the record is writable.
- Card 10 — ordered steps advance behind committed completions: step i+1 is born only from hearing step i drained, behind that step's committed completion, so a duplicate completion cannot start the next step twice. A cancellation is a settlement proposal that fences the active step, read from the record at the reading the claim is exact at.
- Card 11 — recursive child workflows: a step dispatching a child member asks its Task, which cuts exactly the subtree its recipe bound under that child; the child proposes its own admission and runs only behind it, and a fence reaches every level. One engine at every level, and no barrier counts settlements.
- Card 12a — restore one Work and redeliver its admitted request: restore is not a mode. A workflow asks the record what it holds about its execution before anything is born; an attempt claimed by a dead incarnation is dispatched again with the same attempt id and no new claim — a transport redelivery, not a new attempt.
- Card 12b — retries under a restart, and the fence: the committer runs under a lease that is rows of its own record (`lease.taken` / `renewed` / `released`), whose token is the row's own and never its position, so the fence is atomic with every write by construction. A fenced holder commits nothing and answers `fenced`; a reboot never mints an attempt the record does not have.
- Card 12c — uncertain external effects reconciled at a restart: recovery asks the admitted performer kind's reconciler for the identity that kind selects. An adapter that acted hands back what it recorded and that is the outcome; one that attests it never acted, or is documented idempotent, is redelivered; one that cannot say leaves the claim `unknown` and the attempt waits visibly until `hale dna effect resolve` says what happened.
- Card 13 — recovering the tree from the record: a restart rebuilds an execution from the record alone and births the Task from the admission the projection accepted, so a restored execution runs the recipe it was admitted with whatever the catalog offers now. A task never admitted, or admitted with a recipe it cannot read, is refused out loud rather than invented.
- Card 14 — exact plan and Mutation correlation: which exact Work and attempt asked for a plan and produced a Mutation is persisted with the request as asked, so a restart resumes each Work under its own plan with the request it was asked with. A Mutation with only its request row was in flight, fails once, and its id is never minted again.
- Card 15 — edit outcomes through Work completion: an edit leaf is performed by the assembly and its outcome comes back for that exact attempt, under two output contracts that are never reinterpreted — a prepared candidate, and an applied or approved one, which waits on the review of its Task's candidate. An attempt asked again is answered from the Mutation bound to it and edits nothing twice.

**Owners, ledger, funding and leases.** An organism has three memories, and every row kind has exactly one home.

- **The ledger (GH #646 stage 1, #650).** The Structure is the codebase, the record (git) is how the organism changed and was allowed to, and the **ledger** (the store) is what it did today — intents, tasks, decisions, bills, money, schedules fired, concerns, effect claims, liveness. The table is `dna::memory_of`; routing is versioned and is a fact of the record, never a build's opinion, and there is no dual-write. The core links no database driver and a head never holds database credentials.
- `hale dna ledger adopt` moves an organism to routing 1 in four idempotent steps, with no body live and the record synced; `hale dna ledger abandon --why` returns it to the record alone with nothing lost; `hale dna ledger` says where the day's work lives. Adoption stayed closed until every operational write path went through the service at stage 3, `HALE_DNA_ADOPT_UNGATED` being the fixtures' way past the gate.
- **Coordination in the store (GH #646 stage 2, #651).** On routing 1 every lease is a row of the store, including the body lease, so an adopted organism's leases move with it and nothing in its wiring changes. A store lost after activation is handled as the git lease already was: the renewal fails, the fence stops the organism at the margin, and `status` says the ledger is unreachable and nothing is admitted.
- **Heads write through the service (GH #646 stage 3, #652).** An operational row a verb writes from a clone goes to the service in the person's name and is admitted against the record as it is then — a retired person's request is refused, a completion for a task handed to someone else is refused — and every request carries an id minted at capture, so one submitted twice lands once and one refused stays refused. A head that cannot reach the service queues the request and says so: `hale dna queue` lists what waits, `hale dna queue submit` sends it in capture order, and every verb drains the queue first. Nothing on a head is authoritative.
- **Evidence and the export across two memories (GH #646 stage 4, #653).** Receipts are placed by what they evidence, and sync applies redactions and candidate drops over both memories read as one, so a redaction after adoption removes a body filed before it. The books export reads the ledger through the service, never SQL of its own, and the recovery scans a restart runs are exercised over the two memories as well as over one.
- **Handoffs service to service (GH #646 stage 5, #662).** `Exchange` is one record's mailbox in another, idempotent by the envelope's kind and entity. A connection whose url is a service exchanges service to service and one whose url is a record's remote exchanges as mailbox refs; a handoff published whose envelope the peer does not hold is delivered again, once, and a delivery that cannot reach the peer records nothing and says so. Settlement stays on the receiver's acceptance alone.
- **The owners map, and admission reading it (GH #664, stage B2).** `dna/org/owners` names an owner per position and each owner's members; empty, the organization is the one owner and nothing changes. Over a shared record a body says which owner it is (`HALE_DNA_OWNER`) or `run` / `dev` refuse to start it, and it admits an intent only for a position its owner holds, refusing one offered for another's by name. `hale dna ask --to` takes an intent for another owner's position to the record instead of the membrane. A candidate that changes the map is approved by every owner it affects, through one of its members, and one rejection settles it.
- **A lease per owner, and an epoch on every write (GH #665, stage B2).** The body lease is a row of the store per owner, so two owners' controllers run side by side over one record, and a shared record runs no body until its ledger is adopted. The lease's token is an epoch, held through renewals and raised by a takeover: the host hands the body its lease and epoch (`HALE_DNA_LEASE`, `HALE_DNA_LEASE_TOKEN`) and the service refuses `fenced: …` a write under a lease not held, held at another epoch, or expired.
- **Ids and effect claims carry their owner (GH #666, stage B2).** Over a shared record a controller mints in its own namespace (`acme:t3`) and restores its counters from its own births alone, and the store decides a first claim — a birth under an id already born answers `claimed`, so the assembly mints the next id instead of refusing the intent. With one owner nothing is prefixed.
- **Transfers between owners inside one ledger (GH #667, stage B2).** A plan that hands a job to another owner's member appends a transfer request in place of the hand-off, and the Task waits. `hale dna task accept` by a member of the owner named is admitted only in such a member's name; that owner's controller then hands the Task in its own name, so completion is admitted in the assignee's name as before. Handoffs between records are unchanged.
- **Funding (GH #668, stage B2).** A `Grant` names who pays, inherited from the ceiling, and a child naming another funder than its ceiling's is born wider. Each allocation is reserved once through a claim kind the ledger's unique constraint enforces; every attempt is retained, a competing settlement is read rather than doubled, and a compensation is authorized by name and never an implied rollback. A purchase two owners fund is two reservations that may not both land, visibly.
- **Who hosts the shared record's service (GH #669, stage B2).** The owners map names the host, and changing it affects every owner. The host gives each owner a key (`HALE_DNA_OWNER_KEYS`), a head names its owner and key on every append, and the service admits a write in a person's name only from a head of the owner that person belongs to. A service given no keys serves as before.
- **Three smaller ledger items (GH #646).** An intent now names who asked, in the row's body, because an ask entering through a head that shares a clone with a live body goes over the membrane, where the person's name used to reach the assembly and stop (B1). A refused spend is a money row of the ledger — the routing table always said so — rather than the authority refusal a grant born wider than its ceiling gets in the record.
- And two residuals of the three memories: `hale dna receipt redact` on an adopted organism now submits its row exactly at the revision the hold was read at, GH #636's rule having held for the record and not the ledger; and the ledger and lease stores redial the database once and retry a failed statement, as the protected store has since GH #637, so a database restart no longer leaves the knowledge service refusing every ledger write until it is restarted.

**The record.**

- **A reconcile never rewinds the record, and an append never gives up (GH #603).** `hale dna sync`, and the host every tick, reconciled a diverged record by rewinding the live ref to the remote's head and re-appending the local-only events behind it one compare-and-swap at a time — so for that whole window the record had lost rows it held a moment before. The organism's journal also gave up an append after three lost races while every caller dropped the result, so a Mutation's applied and restart rows simply vanished and the Mutation was never retained.
- The reconciled chain is now built beside the ref and swapped in once; a lost race is retried until it lands; a local-only event with no row refuses the reconcile instead of being skipped; and the host keys what it has relayed and handled by a fact's content and its place among identical rows, not by a sequence number a reconcile renumbers. The issue's first description — "a single approval arrives twice" — was wrong: the second is a reapproval, refused as already applied, by design.
- **The record has an API of its own (GH #646 stage 0, #649).** `Record` is a new interface in the core — chains of rows appended under compare-and-swap, a row built beside every chain, bodies by digest, cells by version, pointers, families, signatures, settings — whose vocabulary never names git. One implementation is over git plumbing and one is the fixtures'; the row-commit sequence used to exist three times and now exists once, and the built host binary is untracked and ignored.
- **A reconcile rebuilds only what this clone may sign (GH #639).** Under signed trust every local-only row is read before any chain moves: a row this clone signed is re-signed with its commit naming the original, a row never signed is rebuilt unsigned so a reconcile upgrades nobody's provenance, and a row signed with another key refuses the whole sync, naming the row and the key and saying what keeps the work. Before, a diverged sync re-signed every local row with the syncing clone's key.
- **Candidates are kept.** The gateway points a ref at a candidate when it commits it and sync carries the pointers, so a refused or revised change stays readable as a diff after its worktree is gone. `hale dna candidates` lists them with their Review's state and shows one from its Review's base, and `hale dna candidates drop` stops keeping it here, at the remote and at every clone's next sync, as a redaction removes a receipt.
- **A Task's birth is in the record before anything runs (GH #604).** `ask` used to mint the Task, run it, and journal the birth afterwards, so a crash in between left an intent the record could not explain. The id is minted, the birth appended, and only then is the Task born; a birth the record refuses stops there and nothing has run. On restart a Task born and not settled re-enters from its last durable state — under the plan in the record when there is one, never replanned — and a pre-rule intent with no birth is noted and never re-offered, because work may already have run.
- **People are positions.** A plan names an assignee and the hand-off carries it as data; `hale dna task done` is admitted only in the assignee's name and tells anyone else to reassign, `hale dna task reassign` is a row naming both, and `hale dna retire <who> --to <successor>` transfers every handed Task the person holds and refuses while they hold work and no successor is named. The charter now says who the Board is: whoever holds the record's authority, one person at any level.
- **A completion meets its acceptance condition (GH #604).** When a person's job names its obligation the organism binds the acceptance practice in force at hand-off, so a later change of practice leaves cases already handed under their original contract. `hale dna task done` on such a Task needs `--evidence <digest>` — a receipt the record holds — or `--exception <why> --authorized-by <someone else>`; a note alone is refused, and a completion with neither is labelled human-reported, because a note never verifies anything.
- **Effects and the admission boundary (GH #604).** The claim on an effect key is exclusive: two writers may both request it, but only the first dispatches. An effect requested and never resulted when the organism stopped is resolved by evidence at restart or marked unknown, and the gate refuses an unknown effect until a person resolves it. `dna.trust = signed` makes the host's relay an admission boundary — a record-originated row is relayed only when its commit carries a signature git verifies — and the default trusts every writer, as before.
- **One body per record (GH #617).** The host takes a body lease before it builds or runs anything, at the record's remote when it has one, so two hosts on two clones fence each other through git's own compare-and-swap (`--force-with-lease`). It lives 30s, is renewed every 10s and is asserted at the top of every tick; a second host is refused with who holds the lease. `hale dna body` reads it, `hale dna body claim --force` takes a live lease of a body that is gone, `hale dna body release` gives it up, and `hale dna body start|stop|logs` reach the unit.
- **The body on a server, and its secrets (GH #617).** `hale dna body provision <user@host>` brings a body up over ssh in order — the toolchain installed at the version `hale.lock` pins and verified, the record's remote cloned, the database, then a systemd user unit supervising `hale dna dev` — and stops before writing anything when the record has no usable remote or the host lacks what it needs. `hale dna secret set <NAME>` reads the value from stdin into a mode-600 env file on the body (a `NAME=value` argument is refused) and the record gets `secret.rotated <NAME>` and no value. A host that starts with none of the credentials its catalog names says so at once.
- **`hale dna profile` prints the organism's combination** — record, body, head, fleet, knowledge, trust, github, connections — detected from the pieces, never from a stored label, and `status` gains `profile:` and `body:` lines. `hale dna new --profile local|remote-body` sets the pieces at scaffold time.
- **Protected evidence (GH #606).** A receipt has a data class: a `customer` or `confidential` body never becomes a git receipt but goes to the knowledge service, encrypted at rest under `HALE_DNA_RECEIPT_KEY` — `std::crypto` has no cipher, so the vetted implementation is the database's. With no service the body is withheld and nothing holds it; the record keeps the digest and class either way, so `sync` never carries a protected body. `hale dna receipt file <path>` files the mail or the document, and reading a protected one names a reader and a purpose: the service answers only when a disclosure row names both, and appends a read or a refusal either way.
- **Retention (GH #606).** A legal hold (`hale dna receipt hold`, `release-hold`) stands until released and refuses redaction meanwhile. `hale dna receipt redact` removes a body and appends who, why and under what policy; every sync deletes redacted receipts' refs from the clone and the remote, so other clones drop them at their next sync, and the service answers a read of a redacted body with 410 naming who and under what policy. The record keeps the digest, and a copy made outside the record cannot be recalled.
- **The prompt is a receipt (GH #611).** Every model call's row named a prompt digest and a context digest and the text was never kept, so a reviewer could prove a prompt was sent and could not read it. The evidence now carries the prompt and the context as sent, filed content-addressed under the digest the row already named, and `hale dna history <attempt>` renders exactly those bytes under each call — never a re-composition. An organism whose receipts are a local file store files nothing, and a `customer`-class call files no body.
- **Schedules (GH #610).** The org chart declares schedules in its `birth()`: an interval or a five-field UTC cron, refused at declaration when malformed rather than when it would first fire. Firing is an ordinary ask in the schedule's name, planned, handed or proposed, and reviewed like any other; a schedule never fires while the last Task it fired is open, and the skip is a row. `hale dna schedule pause` and `resume` are rows in your name from any clone, read at the organism's tick, and the optimize pass is itself a schedule.
- **Grants delegate money, counterparties, routes and time (GH #605).** Every field defaults to none, so an existing grant spends nothing; containment covers them all and a child born wider is refused at birth naming the field. The remainder is read at a revision and the row lands only there, so two children cannot each spend the same remainder; a contraction or a revocation advances an epoch that fences an older admission. The generated law adds `money_only_through_the_substrate`. The recorded model tape also gained an occurrence key, so a retried Work planned again with an identical request no longer overwrites the first answer.
- **The knowledge store is scoped by its record (GH #613).** The store's tables carried no record identity, so two records pointed at one Postgres read each other's ideas and advanced each other's watermark. A store is now opened for one record, named by the sha of the journal's first commit, one schema per record; a schema naming another record is refused, and so is an unscoped store from before this change.
- **Cross-record handoff (GH #615).** `hale dna connect` proposes a connection to a separate record as a Board Review, in force once someone other than its proposer approves it, and `hale dna disconnect` closes it. `hale dna handoff` writes one row into the other record carrying its origin, author, source digest, lineage and purpose — a receipt crosses as its digest, never its body, and a class the connection does not carry is refused at the edge. The receiving record admits it only under its own connection back, `hale dna handoff accept` accepts it, and `hale dna handoff sync` reads acceptances back, so a Task handed across settles only then.
- **An attributed external decision (GH #616).** `hale dna task decide` records a decision someone outside Hale made, as a row in the reporter's name naming the decider, the channel, the evidence and the Task: only the assignee may report, the decider is never the reporter, and the evidence must be a receipt the record holds. Whether it settles the Task is its obligation's acceptance policy, a Board-ratified practice; accepted, the Task is shown as a report and never as a verdict, and otherwise it stays handed with the reason.
- **The principal source (GH #612).** `git config dna.principal oidc` makes `hale dna ui` a hosted head: nothing is served without a session, and a session comes from OpenID Connect's authorization-code flow. The head exchanges the code at the issuer's token endpoint itself, over TLS, which authenticates the ID token in place of an RS256 signature the standard library cannot verify; issuer, audience, expiry and nonce are checked. A subject maps to a member and an unmapped subject gets no session; a verdict or an intent from the page acts as that member whatever the form says, `hale dna ask` gains `--as`, and `local` stays the default.
- **Each request in the record is answered by its own answer (GH #689).** The host treated a relayed request as answered when *any* answer for the same entity appeared after it — and a concern's entity is its source, so concerns from one worker shared one entity, one was never relayed, and the relay-again rule never fired for it. A concern now carries its own request id, named in the row that answers it and compared whole, so an answer to `c10` is not an answer to `c1`. One request is one concern: a redelivery writes one raised row, and an id reused for a different concern is refused in a row that is itself that request's answer.
- **A publish the membrane loses is not a lost fact (GH #682, PR #684).** The membrane client exits once it has handed a fact to its binding, and under load the organism could miss it with nothing noticing. Delivery is now confirmed by the answer in the record: `hale dna ask`, `review`, `hale dna concern raise` and `practice propose` beside a live organism write their row before publishing, the host relays a row again while it stays unanswered, the organism admits an intent once by its id, and the host reports an observation again until the organism records it.
- **A concern's occurrence survives a failed read of the record, and a restart (GH #748).** A reload emptied its rows and refilled them from three subprocess reads whose failure comes back as an empty string, so one failed read left the organism holding *no record at all*, in silence — and a source's third concern was counted as its first, never reached the threshold and never became a proposal. The chain only grows, so a chain that reads back shorter than the rows already held is a read that failed, not a shorter record: the rows, the head and the text are left as they were, the refusal is counted, and the next refresh tries again. A concern the organism owed when it stopped is proposed once when it comes back.

**Knowledge, membrane and hosting.**

- **A body's infrastructure and transport are implementations behind the core's interfaces (GH #647).** `Infrastructure` (a body's knowledge database, its supervisor, its credentials) and `Transport` (how a head reaches a body) are the core's; the reference implementation in the host is compose, a systemd user unit and an env file, reached over ssh or, under `HALE_DNA_TRANSPORT=local`, in this machine's shell beneath `HALE_DNA_TRANSPORT_HOME`. Nothing in the host spells `ssh`, `systemctl`, `journalctl` or `docker compose` outside that one file, and a test keeps it so.
- **The GitHub membrane sits behind a `Forge` interface (GH #648).** `Forge` is the vocabulary of a code-review host — open a review, read verdicts with the forge's own key so each is admitted once, comment, close. A GitHub implementation, a file-backed forge for fixtures and a no-forge bare remote implement it; `hale dna github sync` is written against the interface and appends the same rows whichever forge answered, and `hale dna profile` names the forge found.
- **A person proposes a practice for the Board (GH #602).** `hale dna practice propose <name> --text <text>` gives a person a way to propose a practice, which used to be possible only from the toolchain's seeded design or a concern threshold; it is carried over a new membrane topic or relayed as a row. The organization proposes it as a named practice for `org` with the ordinary Board Review, and `hale dna practice` lists named practices with their state. The membrane client names the route only when publishing a practice, so organizations scaffolded earlier keep working.
- **`hale node` and the host** take their bus configuration on the child's own environment rather than through a mutated process environment (GH #887, above), and `hale dna board` still reads everything above out of the record.

**Fixtures and tests.**

- **A fixture takes a port it has bound (GH #873, #909).** Ten call sites across seven fixtures computed a port arithmetically and took it on trust, so a collision with any other listener on the box arrived at the fixture as "the service never came up" — the shape GH #795 reported and GH #869 could not reproduce. `dna::free_port(index)` binds each candidate on the loopback before returning it (the bind *is* the check), steps to the next when the bind is refused, stays clear of the fixed ports the OIDC fixtures hold, and puts each refusal's errno on stderr.
- **A fixture cleans up after itself (GH #873, #909).** Fixtures kept their scratch roots on the pass path, not only on failure: one box held 953 `/tmp/dna-*` directories totalling 2.2 GB, and fifteen fixtures had no cleanup at all. Fifty-four roots now come from `dna::scratch_root(what)` and are removed by `dna::scratch_done(root)`. A slice sweeps what its own fixtures did not reclaim, recorded under its own suite tag so a neighbouring slice or another checkout is untouched, and only a path shaped like a fixture's root is ever acted on. `HALE_DNA_KEEP_SCRATCH=1` keeps everything, read by the fixture and by the sweep so the knob is not undone by whichever runs second.
- **The DNA suite's leftover-process guard blames only its own slice (GH #872).** The guard grepped `ps` for `/tmp/dna-` across the whole machine, so under the documented parallel invocation the first slice to reach its guard was failed by a neighbour's live organism — and by any other checkout's DNA run. One sample during a parallel run found 31 live processes belonging to four slices at once. A slice now stamps every process its own fixtures start (`HALE_DNA_SUITE_TAG`, carried through every generation by `execvp`) and the guard keeps only those, reading `/proc/<pid>/environ`; each line names the root that owns the process ahead of its pid.
- Where there is no procfs to read an environment from, the old unscoped filter stands rather than no guard at all: it over-blames where it cannot attribute, which is a failure a developer can see, and a silent pass is not. `CLAUDE.md` now says the suite's slices may run in parallel, and why — 196s parallel against 661s serialised, so the run a developer wants was exactly the run that falsely failed.
- **Two fixtures wait for their own conditions (GH #795).** `books_slice` and `two_owners` failed on loaded runners at a different phase boundary each time, because assertions ran immediately after a wait for a *different* event and bounds were sized for a machine with the organism to itself. Every assertion that follows a wait now waits for its own condition, the one fixed sleep is gone, and "the same record gives the same export" stops the organism first so its two exports read one static head.
- Every bound is a named constant times `dna::wait_scale()` (`HALE_DNA_WAIT_SCALE`, default 1, never below 1), which the suite sets to 2 for these two fixtures alone. Measured under starvation rather than argued from reading: pinned to one core beside spinning processes the fixture takes three to fourteen minutes, and past that the answer is not a larger number but the slice timeout that is already there.
- **The first books slice runs end to end (RFC #602, stage A).** `dna/acceptance/books` is a capture-only intake: it watches an inbox, files each bill as evidence, and for a bill without its receipt asks the organization for the follow-through and raises a concern. It posts nothing and keeps no ledger, and an export mode reconstructs a week of spending and obligations from the record alone, as open CSV with Decimal totals and the record head.
- The fixture runs the RFC's first-slice gate against a live `hale dna dev` on synthetic documents with scripted models: bills captured and handed to the payer, a completion recorded while the organism is down honoured without replanning, a Task closing only with its evidence or an authorized exception, three omissions leading the optimize pass to a reviewed change that the Board ratifies as a new version of the practice, an older case closing under the version it was handed with, and the week's export totalling five bills with no obligation open and no money moved. The scripted-model surface gains rule lines, so a scenario can script a plan, a proposal and "nothing further" without a model.
- **A forced body claim takes an expired lease too.** `hale dna body claim --force` took the lease of a body that was gone only while that lease was still live, so an operator whose reading and claim straddled the 30s TTL was told to ask again. A body that died a second ago and one that died a minute ago are the same body: a forced claim takes both. Without `--force` an expired lease is reported, as the status line reports it, and left to `hale dna run`.

### Language and checker: check and build read one vocabulary

Underneath the DNA work, the compiler theme of this release is that `hale check` and `hale build` stopped disagreeing. Dozens of programs used to pass the gate and then be refused by codegen, with no file, line or caret, in a vocabulary from a layer below the one the author was working in. Every item here moves one of those answers up to the checker, at the span the author wrote.

- **`Time` is a value (GH #607).** A `Time` is i64 nanoseconds since the Unix epoch, UTC, where it used to be a string-shaped placeholder that could not compare, order or shift an instant. A literal — `` `2026-05-08T12:00:00Z` ``, with an optional fraction of up to nine digits — is parsed at check time, and an offset such as `+01:00` is a compile error rather than a local time read as UTC in silence.
- `Time ± Duration` is a `Time` and `Time − Time` is a `Duration`; any other arithmetic with a `Time` operand is refused naming the two forms, instants order and compare, and `to_string` renders ISO-8601 with the fraction only when it is not zero. `std::time` gains `current()`, `iso8601`, `parse_time`, `unix`, `nanos` and `from_nanos`; `now()` still answers seconds, `time_from_unix(n)` now returns the instant, and on the wire a `Time` is the same 8 bytes as an `Int`.
- **The strict-identifier rule applies on the build path too (GH #911 B1, #846).** The F.18 strict-callee rule (GH #583) was on for `hale check <dir>` only, so `hale build` and `hale run` refused a call to a bare name nothing declares from codegen — with no location, and offering a compiler-internal spelling as the fix. It is now on wherever the compiler has the whole program: `build`, `run`, `test`, `replay` and `hale lsp`.
- `hale check <one file>` keeps the permissive reading, because the sibling it was not handed may declare the name — the line is drawn by what the command was given, not by which command it is. The checker's whole-program library entry holds the whole-program rules too, so a Rust embedder is no longer answered to a weaker standard than the CLI applies to the same bytes; one that deliberately holds a fragment asks for the bundle entry instead. Nine rows leave the check/build divergence ratchet, and `hale bench`, which typechecks nothing at all, is named in `spec/testing.md` as the one command still uncovered.
- **An unresolvable qualified name is a located error (GH #911 B2, #803).** `zz::f()`, `zz::T { }`, `zz::E::V` and `zz::T` in a signature passed `hale check` and `hale verify` and then died in codegen; `let t: zz::T = 1;` was refused by neither layer. The checker now holds the rule in every position a path can stand in — an annotation, a call, a const, an enum variant, and a struct or locus literal.
- Two mistakes get two messages: a head that names nothing reads "`zz::f`: `zz` is not an import or a type of this seed", and a head an import does answer with a name the library never declared reads "`b::Greeting` is not declared by the library imported as `b`", with a did-you-mean and the library's own surface — the shape PR #819 had to run through `build` to see at all. It is gated on the whole-program flag, and permissive where resolution genuinely stops: `std::`, an import the bundle never resolved, and a head that names a declaration.
- The rule refuses nothing the build accepts, which needed one table: `time::sleep` and `time::monotonic` are still lowered unprefixed, so they stay exempt. Every *other* dropped prefix now reports codegen's own answer at the path — "`env::args_count` is unresolved — did you mean `std::env::args_count`? The stdlib lives under the `std::` prefix" — instead of at the end of a build.
- **Generic loci and annotated generic-struct literals typecheck as the monomorph codegen builds (GH #911 B5).** `let h: Holder<Int> = Holder { };` and `let c: Cache<Int, String> = Cache { cap: 2 };` were refused with *expected `Holder_Int`, got `Holder`* while `hale build` lowered, linked and ran them. The checker now takes the type arguments from the declared type at the site — a `let` ascription, a declared return, a field or param default, a data type's field — exactly as codegen does, for generic types and generic loci alike.
- Three shapes are refused instead of failing late. An un-annotated generic literal (`let b = Box { value: 1 };`) is a located error naming the template, its parameters and the annotation to write, since neither layer infers parameters from field values. A wrong-arity instantiation reports *generic type `Box` takes 1 type argument, not 2* at the annotation. And a mangled monomorph name (`Cache_Int_String { }`) is refused with the spelling that works rather than *unknown type*.
- **Generic-argument tokens and statement-position builtins agree between the layers (GH #911 B3, #907, #845).** `type Holder { b: Box<Bytes>; }` typechecked and then refused to build: the mangler minted monomorph tokens for seven primitives while the checker had no opinion, so `Bytes`, `BytesView`, `BytesMut` and `StringView` were accepted at check and refused at lowering. They have tokens now, and `Uint` — which has no codegen representation in any storage position — is refused at the argument's span with the supported set named. Both verdicts come from one shared table rather than two copies.
- `Int(3);`, `len(s);` and `min(1, 2);` in **statement** position checked clean and died at build with ``unsupported in codegen v0: builtin `Int` ``, because a bare builtin call there was lowered by a different dispatch that knows the four printers and nothing else. Statement position now names the printers explicitly and lowers everything else as an expression with the result dropped, so the two positions cannot know different vocabularies again; an unknown bare name there gets the same message it gets in expression position.
- **Three rulings, refused where they are written (GH #911 B6, #820, #901).** A file belongs to exactly one library: a single-file `import "../lib/helper"` of a file another import already claims as part of `../lib` is now a located error naming the library that holds the file and where that import is written, instead of resolving to nothing in silence. The two spellings of *one* library (GH #763, below) are untouched.
- `target NAME { }` inside a `module { }` is a parse error — it is a program-level build directive and every consumer reads the program's own item list, so one declared a brace deeper was silently inert, and the same `std::process::pid()` call a top-level `target wasm { }` refuses reported `ok`. And a module-nested `fn main` is refused by `hale check`, with a message saying the entry point must be top-level, instead of passing check and failing the build with a spanless `program has no fn main()`.
- **A param default that fresh-constructs its own locus type is a located error (GH #911 B4, #870).** `locus Node { params { next: Node = make(); } }` passed `hale check`, built, and then overflowed the *program's* stack. It is the ring GH #813 named, spelled through a function: every `Node` that `make` builds leaves its own `next` to the same default. The checker reports it at the param — "param `next` of `Node` defaults to `make()`, which builds a fresh `Node`; a locus cannot contain itself by value" — and a ring through several types and factories is one report naming the ring.
- Everything the freshness classification cannot follow stays accepted: an accessor, a method call, a `std::` or cross-seed path, a carrier arm. Such a program may still recurse at run time, which is what `@no_recursion` is the contract for; the rule reports only the rings it can prove.
- **A locus that contains itself by value is a located error, not a compiler crash (GH #813).** `locus Node { params { next: Node = Node { n: 1 }; } }` aborted `hale build` with a stack overflow. The containment graph is over by-value construction and its nodes are (locus, the fields the literal supplies), so a default that spells out every param terminates and is not a ring; a **call** in a default is GH #870's rule above. Codegen keeps a matching refusal on re-entry, since an embedder never runs the checker.
- **Every bundle-level check looks inside a module (GH #825, #764, #884, #854).** `module inner { … }` used to switch off twelve checks for everything in it — `accept` without `release`, cross-pool calls, a second `main locus`, bounded-topic policies, fallback subscribers, nested long-running children, unowned bus subscribers, pool affinity and cross-pool aliasing among them — because each walked the top-level item list while the resolver, the typecheck pass and the effects manifest all recursed. The hot-path allocation lint was the same defect a step earlier (GH #764, PR #823), and this is the survey that fix called for. One shared walk now flattens module nesting to any depth.
- Half of those are index passes, and they failed *open*: a walk stops, a lookup misses, and the miss reads as evidence of safety. `compute_pool_of_locus_type` returning an empty map switched the whole F.31 layer off, finalization included. One check failed the *other* way and is fixed with them: a `bindings { }` block inside a module read as "this topic has no transport", so a legal `or wait` (GH #255) was refused.
- Codegen had the same omission (GH #884): a module-nested type, locus, enum variant, free fn or interface was build-time-unknown, and a module-nested topic was read as bus-inert, so its handler never ran. Across an import (GH #854) a module-nested declaration now gets a mangled name and a rename row in both directions, and a qualified path written inside a module is collapsed like a top-level one. `fn main` is the single deliberate exception and stays top-level-only.
- **An unknown bare type name is a located error with a suggestion (GH #877).** `fn helper() -> int { }` passed `hale check` and died at `hale build` with `unknown type name 'int' in signature` — and because an unresolvable annotation typed as `Unknown`, it also silenced every check that would have used it. The message now offers the primitives first, then the program's own type names and the generic parameters in scope.
- Every annotation position is walked, not only the signature: parameters and returns, the `fallible(E)` payload, struct fields, enum-variant payloads, alias targets, `const` ascriptions, locus `params`, `capacity` cell types, lifecycle, mode and `on_failure` parameters, and `let` ascriptions — including inside `[T; N]`, `(A, B)`, `bounded[T; N]`, `fn(A) -> B` and a generic argument. Generic parameters, interfaces, mangled monomorphs and the nine compiler-synthesized channel type names are asked for explicitly, so the rule stays at least as permissive as codegen.
- **A bus subject's payload must be a type the bus can carry (GH #876).** `bus { publish "org.metrics" of type Int; }` typechecked clean and could not be lowered on either side. The checker now answers at the `of type` clause's own span, naming the accepted set — a user `type`, an enum with at least one payload-carrying variant, or `std::bytes::BytesView` — and carrying the repair that fits the shape. PR #458's bus-payload rule compared a handler's parameter against the declared payload and never asked whether the declared payload is a payload at all.
- **A builtin never silently beats a declaration you wrote (GH #880, #863).** A free `fn abs`, `to_string`, `Int` or `Float` checked, built and ran the *builtin*, with the author's body as dead text and no diagnostic anywhere; `fn min` and `fn max` did the same (GH #863), and `fn print` / `println` inverted it by capturing the Hale-source standard library's own calls. A free `fn` may no longer take a bare builtin's name, refused at the name with the per-name reasoning spliced in, and codegen keeps a backstop that refuses the build if a builtin arm is about to answer a call to a name the program declares.
- A locus method, an `interface` method and a `perspective` contract `fn` keep every one of those names — they are reached through a receiver, which no builtin claims. Fifteen names are deliberately left free, each verified to build and run its own body.
- **A user fn shadows the bounded intrinsics of the same name (GH #892).** `fn count(xs: bounded[Int; 8]) -> Int { … }` beside `count(w.samples)` printed the live count and the declaration was dead text; the same for `clear`, `truncate`, `push`, `at` and `set`. Dispatch here is type-directed, so the shadow is decidable at the call site: a free `fn` whose first parameter is the receiver's own `bounded[T; N]` now **answers the call**, at any arity that declaration accepts, in the checker and in codegen alike. Element type and capacity are part of the match, because the stdlib calls those intrinsics on buffers of its own. Method position is untouched.
- **Both directions of the exempt-builtin table are tested rather than trusted (GH #779, #800).** Five names the compiler answers (`starts_with`, `contains`, `eprint`, `check_closures`, `mean`) were missing from the table, so `hale check <dir>` refused three corpus fixtures the build compiles and runs. And nine names the language does not have (`hex`, `panic`, `exit`, `String`, `Bool`, `Bytes`, `Decimal`, `Duration`, `prod`) were listed but answered by nothing, so a call to one passed check and died at lowering; they are dropped and now draw the located unknown-callee error at their own span.
- `Float(x)` goes the other way: specified beside `Int(x)` since v1.x-11 and never lowered, it now has its widening cast and agrees with `std::math::int_to_float`. The purity list of bare names is reconciled with the same rule — eight of its twelve entries are keywords or do not exist and could never stand in callee position. Unrelated nit from the same report: `std::crypto::ecdsa_p256_sign`'s doc comment stops advising a `std::bytes::len` path that does not exist.
- **A pinned locus cannot be created inside a loop (GH #826).** A `placement { }` entry spelled `pinned` gives its field an OS thread whose join record is one slot per instantiation site, so a main locus booted once per iteration orphaned every earlier thread with its arena, its `@form` buffers and its whole child tree. Placement names static resources, so the shape is refused rather than given a per-iteration join (PR #824 had deliberately stepped over pinned entries, and GH #253's join-before-cascade made whether it leaked turn on an invisible internal path).
- The diagnostic sits on the literal with the `placement { }` entry as a related location, fires at any loop depth and inside a `module { }`, and is positional — a loop that *calls* a fn holding the literal is unaffected, and correct, because every call joins its own thread. Codegen keeps a matching refusal for embedders.
- **A placement entry is never silently dropped (GH #890).** `placement { a: pinned; }` over `a: Worker = make_worker();` spawned no thread and said nothing: a factory call lowers no literal, so the one-slot override sat untaken until the next field's turn reset it, and the same drop hit `cooperative(pool = X)`. Honouring it after the fact is not available — the factory's literal has already run birth and `run()` — so every entry must now be consumed by exactly one instantiation, with the diagnostic on the initialiser, naming the shape that lost the entry and spelling the literal form to write.
- **The type alias form `type Name = Type;` parses at all (GH #759).** It never did: the alias/enum split compared token discriminants, so every identifier entered the enum branch and the alias branch below it was unreachable, while the rest of the compiler consumed a variant it could never receive. The form is transparent end to end — an alias is a second spelling of its target in every type position, across an `import` — with a cyclic chain a located error rather than a compiler that follows it forever.
- **A struct literal may be spelled with an alias (GH #831).** Construction was the one position GH #759 did not reach, because codegen reads a literal's path at some twenty sites and half a fix is a check/build divergence. The alias is now resolved once per layer, before any construction site sees it: a chain of aliases, an alias declared inside a `module`, an alias of a locus, of a generic instantiation, of an enum in a variant path, and across an import. The pattern half closed a pre-existing divergence of its own — `match c { C2::Red -> … }` checked clean and died at build.
- **A generic type alias is refused at the parser, with a message that says so (GH #834).** `type Twin<T> = Pair<T>;` is not supported — an alias declares no type of its own to monomorphize — but the first thing the author read was a mismatch between two monomorph names. It is refused at the `<` that opens the parameter list, with the concrete form to write. The two template forms keep their parameters, an alias *of* an instantiation stays transparent, and the parser is the right layer because no later one ever sees the shape and `hale fmt` never has to render one.
- **Stdlib argument types are checked (GH #829).** `std::io::tcp::recv_into(0, 0, 64)` typechecked and then told the author, from the layer below, that `buf` must be a `std::bytes::BytesBuilder`. Five rows now name it — `std::io::tcp::recv_into`, `std::io::tcp::recv_stamped_into`, `std::io::tls::recv_into`, `std::io::tls::recv_stamped_into` and `std::io::udp::recv_into` — so a wrong-typed buffer is a located error in the spelling the author writes. The `Any` rows that stand for a user `@form(vec)` destination (`std::str::split_into`, `std::str::join`, `std::text::tokenize_words_into`) stay permissive on purpose.
- The gate that should have caught it could not see the program: the check/build sweep excluded every harvested program with no entry point, on the reasoning that one cannot be built — which is false, since codegen synthesizes the C `main` itself. 88 check-clean snippets had therefore never been compiled at all; the sweep now appends a synthetic `fn main() { }` and builds them like any other.
- **Struct-returning stdlib helpers are typed (GH #771).** `std::json::string_field(body, "owner").knd` compiled, a struct could be handed to a `String` parameter, and a swapped argument pair read an iterator's fields off a String — eleven dispatched helpers returning one of six stdlib structs had no signature row, so every one typed `Unknown`. They are typed now, `std::http::Request`, `std::http::build_context`, `std::term::size`, `std::str::bytes_view` and `std::metrics` having already been typed through GH #470. `spec/stdlib.md`'s span-cursor arities were wrong and are corrected to `std::json::array_first_span(json)` / `array_next_span(it, json)`.
- **The classic-pool blocking lint reads the effects registry, not a hand list (GH #830).** The pool-stall warning and the dead-receiver error asked a hand list of eleven stdlib paths while the registry next door classified thirty-three leaves `block`, so `std::io::stdin::read_line`, `std::io::udp::recv`, `std::http::get` and thirteen more drew no warning at all — a console locus reading stdin in its `run()` loop on a shared pool being the plainest instance of the stall the lint exists to name. The hand list is deleted and the leaf set is derived from the registry.
- One row is carved out, read off the lowering rather than guessed: `std::time::sleep` chunks into ≤100 ms slices and drains the pool between them, so counting it would have had the lint flag its own advice. The same predicate has a second reader, so a `while true` publish loop paced by a blocking line read no longer draws the unbounded-publish warning, and `std::time::tick` still counts as a throttle.
- **The blocking lint knows `sleep` parks on `async_io` (GH #791).** A sleeping handler on a `where async_io` pool was warned that it stalls the pool and told to add a `@no_block` that does not compile there — the advisory that exists *because* of the sleep-holds-the-pool bug was flagging the fix for it. The placement-implied advisory now runs over `block` minus the leaves that park, enumerated from the lowering beside the runtime primitive that parks each (PR #828). The `block` **classification** does not move: it is a property of the call, not of a placement.
- **An imported type in annotation position is checked like a local one (GH #833).** `let t: lib::Thing = "x";` passed check and died at build, because the rename pre-pass walks top-level declarations only and the one annotation position that lives in a body reached the resolver still spelled qualified. The resolver answers the path itself now (PR #851), so two aliases for one library are one type at check time as they are one type at run time. Both permissive cases are kept: a path with no rename row, and one file of a multi-file seed checked alone.
- **One library under two spellings is one library (GH #763).** `import "../lib"` in one file and `import "../lib/main"` in another produced two library identities and two symbol sets — and because the parse-once set is global, no rename rows at all for whichever resolved second, so `alias::Name` against it died at codegen while `hale check` answered `ok`. A library's identity is now its seed directory, so both spellings name one library, one file set and one symbol set, and `[ffi]` in the library's `hale.toml` is read for both.
- **A qualified path must use an alias its own seed declares (GH #762).** A seed could write `u::f()` for an alias it never declared and resolve through *another* seed's import row, so the library called whatever library its consumer happened to spell `u` — and the same library compiled from a different app called something else. It is now a located check error naming the seed that does declare the alias and the `import` line to add here; the scan is lexical, so it sees every position a qualified path can stand in.
- **An alias is scoped to the seed that declares it, in the build as in the language (GH #746).** Where two seeds bound one alias name to *different* libraries the per-build rename table kept the last binding and both seeds' `u::f()` reached the same library, with `hale check` answering `ok`. Each binder now gets a head of its own and that seed's own references are rewritten to match, so every build in the tree is byte-identical and a contested one can no longer resolve to the wrong library in silence.
- **A module alias and a same-named free fn stay distinct across an import (GH #714).** A seed may declare `import "../core" as core;` beside `fn core(...)` and check, build and run that way — until someone imported it, at which point the mangler rewrote the *alias* to the free fn's symbol. The head of a two-segment path is never a value, so it is now rewritten only when it names one of the seed's own **type** declarations, which is the enum-variant path rule of GH #534.
- **A claim group keeps its own seed's vocabulary through an import (GH #774).** A claim naming a group no declaration in its own seed answers was an error when that seed was checked directly and stopped being one the moment an importer declared a group of that name — the worse half being that a `forbid` written about a seed's own participants was re-pointed at an innocent locus and reported `holds`. Such a reference is now bound to its seed at the merge, in all three tiers, and resolution still never widens.
- A policy seed's `constitution` is the deliberate carve-out: a seed that declares no `main locus` closes no world, so its clauses are adopted elsewhere and its group vocabulary is each adopting entrypoint's to declare — the rule `--matrix` already covers. A constitution in a seed that *does* declare `main locus` binds like the other two tiers.
- **An imported main's inline claims keep their seed's groups (GH #733).** A `main locus` stating its law inline checked clean on its own and was rejected the moment another seed imported it, because GH #382's mangling rewrote every group *declaration* and left the *references* inside a main-locus `claims { }` block alone — so an imported main's law detached from declarations three lines above it. Importing a whole application is what an application's own tests do, so the workaround was to move the claim into a `constitution`.
- **Conventional stdlib spellings point at the builtin (GH #722).** `std::str::len(s)` was refused as an unknown stdlib function with no hint, and where edit distance did fire it misled: `std::string::len` drew "did you mean `std::ring`?" and `std::math::min` drew "did you mean `std::math::sin`?". Edit distance can only reach another name in the same namespace, so it can never bridge a namespace-to-builtin move.
- Nineteen spellings over nine operations now answer with the builtin's call shape — `std::str::len`, `std::str::length`, `std::string::len`, `std::string::length`, `std::bytes::len`, `std::vec::len`, `std::math::abs`, `std::math::max`, `std::cmp::min`, `std::cmp::max`, `std::io::print`, `std::io::println`, `std::io::stdout::print`, `std::io::stdout::println`, `std::fmt::print`, `std::fmt::println`, `std::str::to_string` and `std::str::from_int`, plus `s.len()` / `s.length` / `s.size` on a `String` or `Bytes`. Every row's advised shape is verified against the codegen arity, and an operation with no builtin equivalent (`std::json::parse`, `std::str::concat`, `std::str::to_lower`, the `std::str::parse_*` readers) keeps the plain diagnostic rather than gaining a suggestion that is not a valid call. Reported from a downstream handoff.
- **A reserved word used as a name is reported once, where it is written (GH #725).** Naming a field or a local `epoch`, `where`, `rich`, `tier`, `restart`, `capacity`, `release`, `run` or `of` used to abandon the whole declaration, so the author read a cascade about stray braces and `expected member name, got Rich` instead of the rule. There is now one diagnostic at the word, naming the word and the declaration context, for params fields, locals, loop variables, fn, method, parameter, const, type, locus, perspective, interface and generic-parameter names and struct fields; the parser resumes as if the declaration had been named, and later uses of the same word fold into that one report.
- Underneath it, a parse diagnostic's span was shifted by the file's virtual base a second time, so in every file but the first of a seed a parse error rendered past the end of the file and lost its snippet and caret — which also mislocated every parse diagnostic `hale lsp` published for a non-first file, and every f-string interpolation offset with it. No keyword is unreserved and no escape form is invented. `spec/tokens.md` gains the one hard keyword it had never listed (`resets_per_epoch`). Reported from repeated authoring friction (downstream handoff).
- **`@unbounded` stacks with the effect assertions (GH #723).** `@unbounded` and `@no_syscall` each parsed alone and neither order parsed together, so a fn could not both acknowledge an intentional accumulation and promise which effects it reaches; a downstream handoff wrapped `@unbounded` methods in a free `@no_syscall` fn purely to carry the second contract. The contract decorators — `@unbounded`, `@hot`, `@budget(...)`, `@effects(...)` and its `@no_*` sugar — now parse as one run in any order and apply together. `@ffi` and `@export` say what a declaration *is* and still take the slot alone.
- Two stacks became check-time errors at the offending decorator, with the decorator they collide with as a second location: the same decorator twice, and `@unbounded` against a contract that forbids allocation — `@hot`, an assertion excluding the `alloc` class, or `@budget(alloc_per_call = 0)`. A ceiling above zero composes with `@unbounded`, because the two measure different things.
- **An imported library that fails to parse fails check and verify (GH #765).** `hale check .` and `hale verify .` answered `ok` and exit 0 on a program whose imported library was missing a semicolon, while `hale build .` refused it — so an admission gate built on check + verify was fail-open for any change that broke a library's syntax. Both commands now fail on a parse failure anywhere in the import graph, at any depth, with the diagnostic rendered at the offending file's own line and column (the other half of GH #770's double shift, fixed here).
- **A `type` or a `const` inside a locus is rejected at its declaration (GH #756, #747).** Both passed `hale check` and failed the build with `locus member kind not yet lowered to codegen`, a message naming no line — and nothing could read the declaration either way, since the resolver registered no member. They are now type errors at the keyword, naming the spellings that work: a top-level declaration, in scope inside every locus of the seed, or (for state) a `params` field with a default. `type_decl` and `const_decl` leave `locus_member` in `spec/grammar.ebnf`, with the parser still recognizing the keyword there so the error stays located instead of cascading.
- **A field named after a synthetic locus member is rejected at its declaration (GH #734).** Every locus carries three members the compiler injects — `children`, `k_max` and `draining` — which resolve ahead of anything the locus declares, so a locus that declared one of those names had a member it could never read back, and the failure landed anywhere but the declaration. The three names are reserved at the declaration now, unconditionally; loci that legitimately accept children are untouched, and a `type`'s struct field named `children` stays legal. Reported from a downstream handoff building recursive ownership.
- **An undefined identifier is an error, not a build failure (GH #721).** `println("" + totl);` passed `hale check .` and `hale verify .` with no finding and failed to build with an unlocated `unknown identifier totl`, so an admission gate let a one-character typo through (downstream handoff). A bare identifier in value position that names nothing in scope is now a type error at its own span with a did-you-mean, wherever the program is whole; one file of a multi-file seed keeps the permissive reading. Two scope gaps it exposed are closed with it: a match arm's pattern binders and the implicit `err` of an `or fail` payload were never entered into the checker's scope though both are in scope at codegen.
- **An unknown field in an imported type literal is an error (GH #707).** `alias::Resume { key: "main" }` resolved to the merged declaration for its result type only and never checked its initializers, so a misspelled field was dropped in silence and the declared default constructed instead — `hale check` reported `ok` and the program ran with the wrong value. Imported literals are now checked exactly as local ones are, demangled to the `alias::Type` spelling the author wrote. Reported from a downstream handoff, where the typo sat in a recovery signal and the compile gate was the only thing that could have caught it.
- **`serves` accepts a qualified imported perspective (GH #724).** `locus X : serves lib::Routing` failed to parse, so a library could declare a perspective contract no consumer could implement. All three positions that name a perspective — the `serves` clause, the `perspective(P)` slot type and `reperspective self.<field> as <Impl>` — now take an alias-qualified path resolved through the same rename table as a qualified type or call. A tool holding one seed without its imports treats such a path as opaque, keyed on the alias, so `hale lsp` does not squiggle a program `hale check` accepts. Reported from a downstream handoff probing a standalone routing seed.

### Locus ownership: resolved before lowering

- **Who reclaims a locus is decided before any code is emitted (GH #921 A2, A3).** Ownership used to be decided by seven one-shot flags on the codegen context, each consumed by "the next literal or call lowered" — and every teardown leak and use-after-free the 2026-09 sweep fixed was a flag taken by the wrong node. A pre-pass now computes an owner (`Binding`, `Field`, `FrameTemp`, `Caller`, `Borrowed` or `Placement`) for every locus-producing expression before lowering begins, keyed by a node id carried in the AST, and lowering reads it. All seven flags are gone.
- Four open families went with them. A factory returned through an `if` / `match` / block tail is proven fresh through the carrier's arms. A frame temporary inside a loop is reclaimed per **iteration** rather than once per frame. A factory's result in a `perspective(P)`-typed param field is owned by the field exactly as an `interface`-typed one is. And `or <call>` into an interface- or perspective-typed field takes the owner's mask bit, where the old predicates compared the factory's declared locus against a field that has none (PRs #910, #913, #916, #918 and #919 carried the intermediate steps; GH #896 goes with them).
- A `placement { }` entry is claimed by the instantiation the entry names rather than by whichever literal lowered next — which also closes a scalar-defaulted field leaving a pinned override for the next instantiation anywhere to take. Storage and ownership are separate questions again: a `bindings { }` transport, adapter or codec says "allocate for the program's lifetime" explicitly instead of spoofing the enclosing fn's return type to get both, and a codec locus finally has an owner, so its `dissolve()` runs.
- **An instantiation with no owner is a hard build error** naming the expression. F.39 gives every locus-producing expression an owner before lowering begins, so the absence of one is a compiler defect and not a program error; the shadow mode A2 used to compare the table against the flags is gone with the flags. One table-model defect this found: a bare `=` to a name the frame hands back is the *caller's*, the same carve-out `let` already had, so `fn f() -> Buf { let mut a = make(); a = Buf { }; return a; }` no longer hands the caller a reclaimed locus.
- **A factory result is owned by the node the site names (GH #837).** `suppress_fresh_temp` was taken by whichever proven-fresh factory call lowering reached first, which in `return combine(a, make());` is the *argument*: `make()`'s result was recorded as the caller's though the caller never sees it, and nothing reclaimed it. The decision is now attributed to the node the site names (PRs #835, #882), so every factory result ends with exactly one owner and dissolves exactly once.
- **A factory result returned through `if` / `match` / a block is owned per path (GH #883).** Those forms produce no value of their own, so the first arm lowered took the site's decision and every other arm's result was torn down inside the frame that built it — a use-after-free the caller then read, silent with chunk recycling on. Each arm's tail is now the expression the site names on the path that runs it, a block narrows the decision to its tail, and a composite's ownership stops at the composite rather than passing to its first element.
- **A `let`-bound factory-returned locus is dissolved by its binding's scope (GH #793).** `let c = std::process::spawn("sleep\n30") or raise;` left the child running with PPID 1 after the program exited, its three pipe fds open and the handle's arena abandoned. The rule existed twice over but both halves decide on a call, and `make(...) or raise` is an `or` — so the factories that most need reclaiming were exactly the ones neither rule saw. The owner is now registered on the `or`'s **ok branch**, in a slot the err branch never writes.
- **A factory's result transfers into a param field in every spelling (GH #836, #853, #895).** `Router { quick: make(5) }` never dissolved its child: the frame stood back and the owner's mask bit was set only by a literal init (GH #836). The `or <substitute>` spelling gave its value no owner on one branch and two on the other (GH #853), and a factory whose result reaches an **interface**-typed field was the shape nobody owned at all, since the predicate compared the factory's declared locus against a field that has none (GH #895) — the child and its `@form` buffers lived to process exit.
- An accessor — a fn that hands back a locus its own `let` still owns — leaves the bit clear exactly as before: a missed bit is a leak and an extra one is a double free. Telling one from the other is the same proven-fresh-factory fixpoint that has decided ownership since GH #383 and GH #402, not a new discriminator.
- **A locus created in a loop is reclaimed every iteration, not only the last (GH #815).** A deferred-dissolve slot is one alloca per instantiation *site*, so `while i < 32 { let h = Holder { tag: "loop" }; }` ran 32 instantiations and exactly one dissolve; the other 31 arenas, `@form` buffers and child trees were still live at process exit. Control arriving at an instantiation a second time is proof the previous occupant is dead, so it is now reclaimed before being overwritten, with the same spine the scope-exit flush emits.
- Both the literal and the proven-fresh factory binding are covered; a literal that escapes the loop never takes a deferred slot and is untouched, and a handle the loop passes to an owner keeps its mask bit clear so there is no double dissolve. Only sites lowered inside a loop body emit any of it, so a straight-line program's IR is byte-for-byte what it was, and `let` stays fn-scoped.
- **An unowned locus literal in expression position lives as long as its caller's scope (GH #711, #812, #710).** `serve(Provider { })` — a literal handed to a fn through an interface-typed parameter and retained by the callee — died by a signal at the provider's first retained-field write, and `println("a=", Holder { tag: "hello" }.tag, " b=", Holder { tag: "again" }.tag)` printed `a=again-1 b=again-1`, a silent wrong answer (a downstream handoff). The literal's teardown was emitted at the end of the literal, before the callee ran.
- Ownership now belongs to the position — any expression position, receiver (GH #710) included — so writing a literal inline and naming it with `let` first are the same program (PR #814). A bare `LocusName { };` **statement** keeps its fire-and-forget teardown at the statement boundary, and nothing new is allocated: only the reclaim point moves.
- **The teardown cascade recurses to the leaves (GH #750).** A grandchild held as a param field never drained, never ran its `dissolve()` body, never had its capacity slots freed and never had its arena destroyed, in every binding shape, since the first release that had nested param-field loci — visible only when a later allocation recycled the pooled chunk its pointer lived in (PR #811). Every level gates on its own ownership mask, so a subtree handed in from outside is still torn down exactly once by its real owner.
- **Every owned child arena is destroyed at its owner's teardown (GH #871).** Eight of the 88 example fixtures left a locus arena live at process exit on an ordinary build. A field typed by a contract — an `interface` slot or a `perspective(P)` handle — was stepped over by a cascade that walked locus-typed fields only, so each such param now carries a reclaim pointer written by the instantiation that owns it (PR #894). And a field that cannot hold a locus at all armed the parent-owned flag for the first literal lowered, so a whole tree became the child of an owner with nowhere to put it. All eight now report `0 live arenas`.
- **A `return` in `main` no longer leaks the loci bound before it (GH #789).** The return site flushed main's deferred-dissolve frame, which pops it, and pushed an empty one back — so on the fall-through path the entries declared before the `return` were gone and their `dissolve()` bodies, child cascades and arenas never ran. Any `main` with an early-return guard leaked whatever it had bound. The teardown is now emitted from a *clone* and the frame is left standing; the two exit paths are disjoint control flow, and an entry whose instantiation a path never reached holds a NULL self slot the existing guard skips.
- **The bindings transport is dissolved and its listener joined at exit (GH #893).** A `bindings { T: unix(...) }` entry is sugar for a locus whose `dissolve()` has been the documented way for a transport to go away since GH #233 — and it never ran, in either role, because the prelude used the return-type spoof to get program-lifetime storage and got "nobody owns it" with it. The transport is registered on `fn main`'s frame before the prelude runs, so the reverse-order flush tears it down last: after every user locus has dissolved, after the GH #468 exit quiesce and the cooperative-pool join, and before the global arena destroy. `85-bindings-unix` comes off the sanitizer corpus's leak quarantine, which is now empty.
- **Ownership has a generated matrix rather than hand-written cases (GH #921 A4).** Every teardown leak the sweep fixed was one defect in a different outfit, and hand-written cases are exactly the coverage shape that lets the next outfit through. `ownership_matrix.rs` holds 27 positions × 5 types × 7 contexts = **945 cells** and four oracles on each: `dissolve()` tag counts, `LOTUS_ARENA_RESIDENCY=1` reporting `0 live arenas`, an AddressSanitizer run, and the inline-vs-`let` differential `spec/semantics.md` already promises.
- Every fix above closed a family of it, and `KNOWN_OPEN` is now empty: all 945 cells are held to every oracle. The differential never fired over 315 twinned pairs, which is a result — the open families were about the position, never the spelling. The per-PR default runs a deterministic ~90-cell sample; `HALE_MATRIX=full` runs all 945 and belongs in a nightly job.

### Runtime and codegen

- **A cooperative handler owns its payload across a park (GH #781).** A bus handler on a subscriber placed `where async_io` saw its payload change under it: after any park — a sleep, a socket read, a subprocess drain — every delivery read the last published value, deterministically, and identically under record and replay, so the recording gates could not see it either. The async drain handed the handler a pointer into a stack local that died the moment the coroutine parked.
- A delivery's payload storage now belongs to the coroutine that runs its handler and is released once the handler **returns**, however many parks later, which also closes the spilled-payload leak the same gap caused. Non-parking paths are untouched, and the async fast path pays one memcpy of at most 512 bytes — strictly less than the ring dequeue that precedes it.
- **A message queued for a locus that ended reaches nobody (GH #703).** An accepted child that `terminate;`d in a handler while a second message for it was already queued ran the queued handler on a freed arena and died by a signal, because a queued cell carries the handler and self pointer resolved at enqueue and the registry quarantine only stopped *later* publishes. The runtime now drops a cell for a quarantined self at materialization — the one point every consumer passes through — so the message reaches nobody, as a publish after the dissolve does. A program in which no subscriber ever ends pays one relaxed load per cell.
- **`LOTUS_NO_CHUNK_POOL` makes the sanitizer oracle see arena use-after-free (GH #816).** `lotus_arena_destroy` returns a dying arena's chunks to a thread-local pool with their bytes intact, so a read from a destroyed arena touches memory the process still owns and AddressSanitizer sees no `free` at all — which is how four arena use-after-frees (GH #710, #750, #711, #812) went through this gate undetected. `LOTUS_NO_CHUNK_POOL=1` makes the destroy really `free()` and every request really `malloc()`, and covers the coroutine free list on the same grounds.
- Sanitizer builds default it on at compile time (`LOTUS_ASAN=1`, `LOTUS_UBSAN=1` — PR #875), so no instrumented test can forget it and the guarantee cannot lapse by omission; the variable overrides that default in both directions, which is how a suspected use-after-free is chased without a sanitizer rebuild. Ordinary builds are untouched. The anchor-retire free lists and the recognition-class cell pools recycle memory interior to a live chunk and are deliberately not covered.
- **A non-escaping `[c; N]` is a stack slot with one memset, not N arena stores (GH #767).** A free fn has no value arena of its own, so a fixed scratch table inside a helper was per-call growth the calling loop never reclaimed: a `[0; 1024]` table in a helper called 200,000 times took 1679 MB of RSS. A literal `[c; N]` bound to a `let` whose every use is an element read or write now gets an entry-block `alloca` — 73 MB for the same program — with the fill a `memset` for a constant zero pattern and a counted loop past 16 elements.
- The escape analysis is conservative by construction, since a wrong "does not escape" is a dangling stack pointer: every other occurrence of the name keeps the arena path, both walkers match exhaustively with no catch-all arm, the element must be a scalar, and one fn's array literals may take at most 8 KiB of frame in total — one eighth of a `LOTUS_CORO_STACK_BYTES` coroutine stack. `@budget(stack_bytes)` charges an array local its declared extent instead of 8 bytes, and the RSS assertion is read inside the child process (GH #810) rather than from the harness.
- **libm is always linked (GH #808).** `-lm` sat inside the link step's tree-sitter-shim arm, so a `hale` built with `cargo build -p hale-cli` — which cannot produce the staticlib, `crate-type = ["staticlib"]` being unaddressable as a Cargo dependency — failed every `std::math::tanh`-shaped program at the linker with `undefined symbol: tanh`, and surfaced as six phantom check/build divergences. `-lm` and `-ldl` are now unconditional, at the end of the link line (`--as-needed` drops a shared library nothing pending needs at the point it is seen, so the ordering is load-bearing).
- **A missing tree-sitter shim is a located build error.** When the staticlib is not found and the module still references a `lotus_ts_*` symbol, the build refuses with an error naming the artifact and the `cargo build --release` that produces it, at the program's first `std::ts` call, instead of `undefined symbol: lotus_ts_parse_go`. A program that merely merges the stdlib loci that use `std::ts` — `std::source`'s walker — without reaching them is stripped by the prepass and still builds.
- A test-only `HALE_NO_TS_SHIM=1` forces the lookup to miss, so both halves are asserted in a fully-built workspace: the `std::math::tanh` reproducer builds and runs, and a `std::ts::parse_go` program comes back as the located refusal rather than the linker's undefined-symbol wall. `CLAUDE.md`, `CONTRIBUTING.md` and the install chapter now say to build the whole workspace and run `./target/release/hale`.
- **`hale test` dissolves the test's loci before the process exits (GH #717).** `std::test::assert*` called `std::process::exit(1)` from inside the assertion, so a failing test left `fn main` without running any of it — a fixture that had spawned a subprocess and created scratch state leaked both, which is how a downstream handoff ended up hand-rolling an exit watcher. The assertion now records the failure in a runtime latch and returns.
- In `fn main` the latch check branches to one exit block that runs the ordinary teardown — ingress quiesce, cooperative-pool join, bus drain, the whole reverse-order dissolve cascade — and then returns exit code 1, so a test releases what it owns by owning it. The exit-code contract is unchanged, the first failure short-circuits through the latch so there is never a second diagnostic, and an assertion that fails outside main's own frame still exits on the spot.
- **`hale dna` builds the organism's seed once per content fingerprint (PR #931).** `run` and `dev` rebuilt the organization's seed — a handful of project files plus the whole vendored core — on every start and every restart, 3.83s each time and 58.6s of the 212s the eleven slowest DNA integration tests spend. A build is a pure function of what it reads, so the host does it once per fingerprint and copies the binary after that: a first start binds the membrane in 3.91s, a second in 0.15s.
- The fingerprint is a SHA-256 over every file `hale inputs` says the build reads, plus `hale.toml`, `hale.lock`, the toolchain version, the `hale` binary's own size and mtime, every environment variable that changes codegen, and the seed's own path. Binaries live in `$XDG_CACHE_HOME/hale/dna-build`, 32 kept and LRU-pruned; a store and a hit are both a rename, so two hosts building one thing at once need no lock, and a fingerprint that cannot be taken whole is a miss that builds exactly as before. `HALE_DNA_NO_BUILD_CACHE=1` makes every build a miss.

### Standard library

- **A byte view for linear-time scanning of large Strings (GH #720).** A `String` stores no length, so walking one a byte at a time was quadratic — the natural bounded-parser loop over 1 MiB took 13.2 s, of which the one-byte checked slices alone were 7.7 s. `std::str::bytes_view(s)` measures the string once and keeps the count in `v.n`; `std::str::byte_at(v, i)` answers the byte or **-1** for any index outside `[0, n)`, and `std::str::slice(v, lo, hi)` copies a token in O(width), clamped, so a length taken from untrusted input yields short data instead of a read past the end. The same scan is 29 µs.
- These are bytes, not codepoints: `std::str::cp_at` and `cp_size` remain the codepoint walk, and constructing a view is O(n) once, so it belongs outside the loop that uses it. `std::str::range_copy` is public for a scanner that tracks its own length — the materializing member of the `range_*` family and the strlen-free counterpart to `std::str::substring` — and `std::str::byte_at_unchecked` is unchanged, keeping its misuse-is-UB contract.
- A Hale fn whose body only inspects bytes also stops paying an arena subregion per call, through a hand-verified allowlist of scalar-returning, arena-free stdlib paths. Purity is deliberately not the predicate: `std::str::substring` is pure and allocates.
- **`std::json::string_field` tells null from missing from a string (GH #719).** `std::json::find_string_field` returns the raw scalar spelling of a non-String value, so `"owner": null` read back as the four-character String `"null"` — a perfectly good identity name, which is how a parentless node in a downstream handoff's topology adopted a parent called `null`. The typed sibling returns a `std::json::JsonString { kind, text }` whose `kind` is one of `"string"` / `"null"` / `"missing"` / `"number"` / `"bool"` / `"array"` / `"object"` / `"invalid"`, with `text` the decoded content and only when `kind` is `"string"`.
- An `Int` or `Bool` caller can gate `std::json::find_int_field` or `find_bool_field` on the same kind before trusting the same coercion. The permissive scanners are unchanged and their doc comments now point at the typed reader, so every existing caller keeps the behaviour it has.
- **`std::json::valid` and `valid_object`, a bounded linear syntax validator (GH #754).** The `std::json::find_*` helpers are deliberately permissive field scanners and nothing in the stdlib answered "is this text one well-formed JSON value?", so every consumer that must refuse malformed input wrote its own — a downstream handoff carried four hand-rolled validators of the same ~120 lines. `std::json::valid(text)` is RFC 8259 over one value; `std::json::valid_object(text)` adds what callers actually gate on, a top-level object with unique, escape-free keys.
- Three refusals go beyond the grammar, each a value this runtime cannot carry: text that is not well-formed UTF-8, a lone surrogate, and `\u0000`. Nesting deeper than 64 is refused and uniqueness is decided over at most 64 top-level fields, which is what lets the check compare keys where they sit instead of building a table. Both fns are classified pure and allocate nothing at all — a first cut's `[0; 1024]` uniqueness table grew RSS by 1.69 GB over 200,000 calls, which is GH #767's shape. Whitespace runs skip through the SIMD `std::json::next_non_ws`.
- **JSON Unicode escapes decode (GH #708).** `std::json::unescape_string` computed a `\uHHHH` escape's code point correctly and then threw it away, answering `?` for nearly every byte, so an escaped literal never compared equal to its plain form and a value read out of a document could differ from the same value written as a literal. Escapes now decode to the scalar they name, 1-4 UTF-8 bytes, with a surrogate pair combining into the astral scalar; an unpaired surrogate and `\u0000` answer U+FFFD, and an escape whose four digits are not all hex passes through verbatim.
- **Escaping and `Builder` are linear (GH #709).** `escape_string`, `unescape_string` and every `Builder` emitter grew their output with `out = out + piece`, and a String is immutable — so each write copied the whole accumulated prefix and left the old one in arena storage until the call returned, quadratic in time and in retained memory. A downstream handoff generating a document with a 600,000-byte shared value and 512 references reached 23-24 GiB of scratch and exhausted swap. All three now accumulate into one growing byte buffer; `Builder`'s public `buf` param changes type from `String` to `std::bytes::BytesBuilder`, and the method surface is unchanged.
- **A spawned `Child` moves into a locus field with one owner (GH #716).** A service that owns a long-lived subprocess could not keep the handle `std::process::spawn` returned — a locus-typed field takes only a literal — and the only way through was to copy the pid and three pipe fds out and then clear the original so its dissolve would not close them, which is lifecycle internals in application code and a double-close or a leak if written slightly wrong (downstream handoff).
- `std::process::adopt(dest, src)` is the transfer: it releases whatever `dest` held, moves `src`'s pid and fds in, and disarms `src` to the not-spawned sentinels. Order is the point — the field's descriptors stay open until `adopt` replaces them, so the kernel cannot have reissued their numbers to the incoming child. It is infallible, `adopt(x, x)` is a no-op, adopting an empty `Child { }` is the documented way to leave the field empty, and the refusal above now names it with the author's own field name.
- `Child.dissolve()` and `adopt`'s release are one spine now, so both close and reap identically. `std::process`'s own documentation stops claiming a `let`-bound `spawn` cleaned up at scope exit — which GH #793 above makes true, `std::str::clone` of `argv` becoming load-bearing rather than defensive.

### CLI, LSP and diagnostics

- **`hale run` takes the build options, and fingerprints them (GH #922 D2, #904).** `hale run` compiled with default `BuildOptions` whatever it was given and stamped those defaults into the execution identity, so `--dev`, `--target-cpu`, `--link`, `--csrc` and `--target` were unreachable from the documented spot-check `hale run prog.hl`, and a recording said it was made under options its build never used. `run` now parses them with `hale build`'s own parser and threads one set through the compile *and* the fingerprint, so `hale run --csrc glue.c prog.hl` links and runs an `@ffi` program and a dev build under `run` is a dev build.
- `hale replay` takes the same set, because it recompiles the program to admit a recording against it: a recording made under `hale run --dev` is refused by a default `hale replay` and admitted by `hale replay --dev`, and the mirror holds, so the boundary is the options and not the presence of a flag. The `build` flags that report *on* a build rather than change it — `--locality-report`, `--target-cache`, `--strict`, `--wrap-main` — are refused by name for `run` and `replay` instead of accepted and dropped, and `run` refuses `--target wasm32` and points at `hale build --target`.
- `--dev` is set by the flag parser rather than re-derived from `env::args()` inside a debug-info branch, where it was invisible to every other caller and a silent no-op under `LOTUS_NO_DEBUGINFO=1`; `HALE_DEV=1` is read in the same place, so every command that compiles honors it identically. The fingerprint covers the FFI surface, appended only when non-empty so an existing recording keeps the identity it carries. GH #900 had named and refused these flags; this makes them work.
- **An observed iris session dies with `hale` and owns its stdout (GH #922 D1, #905).** `hale run --observe prog.hl | cat` never returned: the session inherited `hale`'s stdout and outlived it twice over, so an orphan held the pipe's write end with nothing left to close it, and a caller waited on a process nobody knew to kill. The program simply *ending* was enough — no signal needed.
- Everything `hale run` starts is now spawned with `PR_SET_PDEATHSIG` and a `getppid` check that closes the fork window, and stays in `hale`'s process group so a group-directed Ctrl-C or `timeout` still reaches it: the compiled program, the `hale iris` session, the `hale build` that materializes the observer, and fuse-hl under it. The session gets its own pipe for both streams, relayed to `hale`'s stderr — moved, not muted — so the program's stdout is the command's output and a caller reading it through a pipe sees EOF when `hale` exits.
- **A stdlib-origin span renders as a note, never a seed range (GH #922 D3, #856).** The leaf of a violated effect assertion is often a stdlib body, and stdlib bodies parse at base 0 in a coordinate space of their own — so a renderer testing the offset against the seed's file windows placed the leaf nowhere, or, in a seed large enough to *contain* the number, as that file at that line: a squiggle on unrelated code, in an editor and in `hale check` alike. Spans carry their origin now, set at the emit site, and no renderer will give one a position.
- `hale check` prints `note: … (in the standard library, io_tcp.hl:118:18)` under the finding, `--json` emits an empty `file` at line 0 with the note in the record's message (a `file` field is a join key and it named the wrong file before), and `build` / `run` / `test` print the same note. The language server publishes no document diagnostic for it at all — there is no range in the open file for a position in another one — and a stdlib-origin related entry becomes a `note:` line instead of a clickable location.
- **Five CLI hygiene fixes (GH #922 D4).** An unreadable *entry* file is a diagnostic rather than a print (GH #903), so `hale test --json` reports that fixture as `"status":"fail"` with the sentence as its message instead of `""`, and the text rendering is byte-for-byte what it was.
- Eleven messages no longer carry a run of spaces mid-sentence (GH #906): a multi-line string literal reflowed onto one line keeps its continuation indent inside the string, which is why the user read ``hale replay: --at takes N or consumer:N                          (positive)``. A test now parses every string literal under `crates/*/src` and refuses a run of three or more spaces between two non-space characters, with the deliberate column tables allow-listed by reason.
- An imported enum prints under its declaration's name (GH #885): `println(sample::Color::Red)` rendered `__lib_lib_types_Color::Red`, a name that appears nowhere in the author's program, from every print path; both renderers now demangle to `Color::Red`, which is what the same enum renders as inside its own seed. `hale test --json` also names each row's file canonically (GH #867), the rule located diagnostics have had since GH #822, while the human-readable `ok <path>` lines deliberately keep the spelling that was typed.
- **No shipped code mutates the process environment (GH #887).** `std::env::set_var` is undefined behaviour in a process that has threads, and three production sites did it so that something downstream would read the value back: `hale model dump` (`HALE_DUMP_MODEL`), `hale run --observe` (`LOTUS_OBS`, planted for the child to inherit) and `hale node` (`LOTUS_BUS_CONFIG`, for the host it then execs). Each now goes where it was meant to — on the child's own `Command::env`, or, for the model dump, a process-global bit that is not the environment and is inherited by nothing — and GH #843's guard now covers shipped code as well as tests.
- **Every command renders a codegen error at its location (GH #848).** A `CodegenError` has carried a source span since GH #241 and only `hale build` used it, so `run`, `test`, `bench` and `replay` printed a Rust variant name and a byte offset into a merged bundle nobody can convert by hand: same error, same compile, five spellings. All five now render `path:line:col: codegen error: message` with the source line and a caret through one helper, and a span-less refusal renders without a position.
- `hale bench` names the bench file: it compiles a temporary copy with a synthesized driver appended and deletes it before reporting, so it re-labels that copy as the bench file and every position is exactly right. Its per-file line also stops prefixing an already-located message with the file name a second time, which had pushed the `path:line:col` off the start of the line where a tool looks for it.
- **`--help` on every subcommand prints its usage (GH #817).** `hale build --help` printed `not a file or directory: --help` and exited 1; `hale init --help` would have scaffolded a project into a directory named `--help`, and `hale lsp` and `hale mcp` started a server on stdio and waited for a client that was never coming. One pre-pass now answers `--help` / `-h` as the first argument after any subcommand, on stdout, exit 0, and `hale --help` points at it. `hale build`'s usage finally writes down where the output goes and which flags exist.
- **Usage lists every subcommand, and flags may precede the target (GH #861).** The top-level usage had drifted three ways: `hale replay`'s whole flag block (`--diff`, `--json`, `--at`, `--feed`, `--allow-unmatched-feed` and the three `--allow-*` admission overrides) was indented under the `hale dna` line, and `hale node` — dispatched since GH #566 F5 — and `hale targets` appeared nowhere at all. A test now reads the subcommand names out of the dispatch itself, so the next unlisted one fails the build.
- `hale build --dev app.hl` failed with `not a file or directory: --dev`, because the target was always argv[2] while the flag parser read from index 3. `build` and `run` now split their arguments with `check`'s rule — the first argument that is not a flag is the target, and a flag is a flag wherever it stands — with the five value-taking flags keeping their arity. `run` stops at the target, since everything after it is the program's argv (`hale run script.hl foo bar`), and names any other leading flag rather than swallowing it; `--observe` works in front of the target as well as in the position GH #527 B3 shipped.
- **A diagnostic's file is spelled canonically by every command (GH #822).** One file had three names: `check` resolved an imported file's diagnostic canonically, `build` / `run` / `test` rendered the path as the resolver reached it (`/abs/app/../lib/second.hl`), and the target's own files were spelled exactly as the command line spelled them — so a CI gate diffing `check --json` against a build failure compared two strings for one file. A located diagnostic now names its file by its canonical path — absolute, symlinks resolved, no `..` — from every command and every channel, at the one rendering boundary that turns a path into a string. `hale lsp` is deliberately unchanged: it publishes against the URI the client opened.
- **An imported file's diagnostic is positioned in its own file on every command (GH #775).** Every file of an import graph is parsed at its own virtual base, and `build`, `run`, `test`, `bench` and `replay` each rendered the merged bundle offset against a single file's text — so the position came out as the end of the file: the right file and message at the wrong place, in the one part an editor jumps to. The base now travels with the diagnostic and one helper renders it, so every command that follows an `import` agrees with `check`. `hale lex` and `hale parse` follow no imports and are unaffected.
- **An unresolvable import is a located diagnostic on every channel (GH #860).** `hale check --json` on a seed whose `import "../nowhere"` names nothing printed to stderr and emitted nothing on stdout — the last empty-stream shape on the check path, for the commonest import mistake there is. It is a `Diag` like any other now, at the line and column of the path literal (which gains a span of its own), with the three tried paths as the body of the message, carried in PR #858's enum and rendered by every channel with no new path.
- **An unreadable or missing input is a record too (GH #806).** `hale check --json /path/that/is/not/there` emitted nothing on stdout and exited 1, so a gate driving `--json` could not tell a bad checkout from a segfault. Each unreadable input is now one record — `{"file":…,"line":0,"col":0,"severity":"error","kind":"io error","message":<os error>}` — from all three sites an input can be lost at. Text mode is byte-for-byte unchanged.
- **Parse errors are records too (GH #777).** `hale check --json` and `hale verify --json` answered a seed that does not parse with a non-zero exit and an empty stream, for every syntactic error there is. They now travel to the same reporting site as every other finding, one record per diagnostic, located in the file that holds it (PR #804) — so an empty stream means a clean seed and nothing else does, which is the property a gate reads it for.
- **A diagnostic at a file's end is published, not dropped (GH #805).** `hale lsp` demultiplexes a whole-program diagnostic by testing its offset against each file's window, and all three windows in the server read the end byte as belonging to no file — so a diagnostic positioned at a file's end was dropped where it was found: nothing published, nothing logged, the editor showing a clean file for a program `hale check` refuses.
- The window test is now one function, inclusive of the end byte, called by the CLI's three renderers and the server's three mappings instead of four hand-written copies of an arithmetic the repo had already written two ways. A zero-length file owns its one position, and the hole was latent rather than observed — which is what makes it worth fixing, since the failure mode is silence.
- **The embedded DNA core carries a digest (GH #726).** `hale dna init`, `hale dna new` and `upgrade` materialize `vendor/dna` from the DNA source embedded in the `hale` binary, so an organism a fixture starts runs the core that binary carries — and two binaries reporting one version can embed different source. A review recorded two mutation results as covered that were measured against a core edited but never rebuilt in. The source set now has a name: a length-framed SHA-256 over the sorted `(path, content)` pairs of every embedded file.
- `hale --version` prints `embedded dna: <first 16 hex>` on a second line, the first line staying `hale <version>` alone because several readers take field 2 of it; `hale dna --embedded-digest` prints all 64 digits, and `--from-tree <dir>` prints what a checkout *would* embed, so unequal means the binary predates the tree. `hale dna status` opens with the digest and names a `vendor/dna` materialized by another build. The build enforces its own snapshot, so a file edited while a build runs fails the build instead of shipping a digest that names nothing.
- **`hale doc --stdlib` and LSP hover render a struct slot as its public path** rather than the `…` placeholder every non-primitive used to get, following the stdlib return types of GH #771.

### Record and replay

- **The coverage report says what it compared for in-process payloads (GH #842).** An in-process flat payload is recorded as metadata only — declared size plus publish identity, since an ABI snapshot would carry heap pointers and uninitialised padding — so `--diff` compares exactly those two things and never the contents, and a flipped `Int` field in such a payload was not a divergence. The report nonetheless said `compared`, with a detail that read like a byte-for-byte match.
- The detail is now derived from the recording's blobs: `payloads: N (sizes and identities; contents not canonicalised)` for metadata-only blobs, `canonical bytes identical` for wire captures, and both counted separately for a recording carrying both. `--json` gains `contents_canonicalised` and `metadata_only` beside `compared`; pass/fail semantics are unchanged. The per-topic canonical codec that would make those contents comparable is GH #947, by ruling.
- **Direct-dispatch publishes record their payloads too (GH #782).** A recording of a fully direct-dispatched workload carried no payload blobs at all: the two direct-call flavors were the only publish sites that never called the recorder, on the reasoning that a closed-world same-thread call re-derives its payloads — which is exactly what makes them comparable, and comparing nothing is not the same as comparing something that always matches. In a mixed program the gap was quiet: a cascade with one queued and one direct subject recorded 32 blobs for 96 publishes and still reported a clean match.
- Both flavors now record through the same writer with the same framing and the same publish identity, so the replay reader learns no second format and no file-format version moves. Recording cost is unchanged when nothing is recording. A recording made before this change is not admitted by a binary built after it, since the payload set is part of what the build does.
- **A successful `--diff` says which schedule categories it compared (GH #728).** The match sentence was built around the consume count, so a workload whose subscribers are direct-dispatched reported `0 consumes` — which read as a *verified* queued delivery schedule where there was never a queued schedule to replay (a downstream handoff read it exactly that way) — and claimed "canonical payloads identical" for a recording that carries no payload blobs, while the 20 subject-aligned events that *were* compared went unmentioned. PR #780's coverage report made the gap visible without closing it.
- A match now names all five comparison categories — public bus events, payloads, queued consumes, async schedule steps, journal reads — and for each either the count it compared or that the category was **not exercised** and why; no category is ever printed as a compared zero. `hale replay <rec> <prog> --diff --json` is the machine-readable verdict, with `result`, `ring_records`, `recorded_prefix_only` and per-category `compared` / `count` / `consumers` / `not_exercised_because`. `--json` without `--diff` is a usage error, and success semantics are unchanged.

### Tests and CI

- **`cargo test` agrees with `cargo nextest` (GH #912 E2).** `cargo test --release -p hale-codegen` exited 101 on `main`, and the doctest run exited 101 for the whole workspace — not a test, but rustdoc compiling a four-space-indented ABI sketch in a doc comment as Rust, four months after it arrived. CI runs the suite only through cargo-nextest, which does not run doctests at all, so nothing could see it; the thread half of the shape was already green.
- A `parity` job now gates both shapes: the workspace doctests, plus the test binaries that really share process-wide state. Scope is measured, not guessed — the whole in-process shape would be the new critical path several times over — so the job takes the binaries that carry a marker saying they share something, computed rather than enumerated, so a new test that reaches for one joins the gate by itself. `CLAUDE.md` states the rule: fix an in-process race at its cause, never with `--test-threads=1`, and fence a doc-comment block that is not Rust.
- **No test mutates the process environment (GH #843).** 23 files in the codegen suite wrapped a build in `std::env::set_var` to turn on a compiler knob — `LOTUS_DUMP_IR`, `LOTUS_ASAN`, `LOTUS_NO_OWNERSHIP_BUBBLE`, `LOTUS_NO_BUS_DEVIRT`, `LOTUS_LTO` — and the environment is one table shared by every thread, so under `cargo test` each write raced the `getenv` of every concurrent build in the same binary. The visible failure was worse than the formal one: the knob applied to whichever *neighbouring* test happened to be compiling, and the paired unset switched it off underneath a build still running.
- Each knob is now a build-options field read at the same single site that read the variable, with every `LOTUS_*` spelling still working as a fallback and the CLI unchanged; one latent bug fell out, `LOTUS_ASAN` having been read twice with only one reading wired through. A guard refuses a new `set_var` outside the one sanctioned helper. Two vacuities closed on the way: an ASan build now checks the artifact really carries the sanitizer runtime, since every ASan assertion in the suite is a negative one, and the devirt differential gained a control that is not a second copy of the static build.
- **RSS regression tests assert growth shape, not an absolute bound (GH #772).** A test asserted `std::process::rss_bytes()` under 100 MB and failed at 137-145 MB where the program's own peak is 30 MB. `ru_maxrss` is not reset by `execve`: `fork` hands the child a copy-on-write duplicate of the parent's address space and the exec folds that high-water mark into the new image for the life of the process — 4.7 MB standalone, 411 MB spawned from a parent holding 400 MB.
- Under `cargo test` the parent is a libtest process running several in-process LLVM builds at once, so every memory assertion in that suite was reading the harness's footprint. Eight tests now measure differences between two of the program's own `/proc/self/statm` reads inside one process; four had been passing *vacuously*, subtracting two readings both clamped to the same harness floor.
- The walk test's guarded defect — a `std::bytes::from_string` copy hidden in each scan helper — is measured as bytes of growth per element across two document sizes, so a regression that inflates both windows equally cannot pass by symmetry. `spec/stdlib.md` and the book state the inheritance rule, since it is a trap for any supervised worker reporting its own RSS.
- The compiled-corpus oracle and the ownership matrix are the standing nets for memory bugs; `hale test tests/hale` and `hale build dna/org` are the standing nets for the language in the language. Both are described in `CLAUDE.md`.
