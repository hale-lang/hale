...
[`ClosureViolation`](../../reference/src/glossary.md#closureviolation)
...
- **Trellis-pair's recovery** — the multi-binary version of
  the supervision pattern. When the analyst and executor are
...
production-shaped Aperio program: a multi-binary
analyst/executor pair that consumes market data, fits trade
kernels, and ships them across processes for line-rate
application.
