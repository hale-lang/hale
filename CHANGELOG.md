...
### DNA: what a seed compiles, and what a store cannot answer (third-round review findings 1, 2, 3 and 4)

- **Untracked source bypassed the dirty-genome check.** It asked git for tracked modifications only, and what a seed compiles is the files on disk: an untracked `helper.hl` beside the reviewed `main.hl` is built by the very seed that expresses the candidate, so the genome checked and built as two files while the candidate was one, and an unfinished one broke the build of a candidate that had passed. Untracked source of the genome's own seed now counts. What that seed does not compile still does not: the binary `hale build` leaves beside every seed, a log, another program's source in a directory of its own, anything under `.hale/` — counting those would refuse every apply in a working project.
- **"Commit or stash and approve again" had nowhere to go.** The first approval settles the Review, so the second one met `already settled` and no route a human has could reach the apply again. The verdict is spent; the apply is not. Approving again on an approved-but-unapplied candidate re-runs the whole gate over the same pinned candidate (`mutation.apply_retried`), and a candidate or base that moved meanwhile is refused as ever.
- **A store that could not answer still answered.** `SELECT 1` proves the session is alive, not that the knowledge queries work: with the schema gone from under it, `/context` returned 200 with an empty idea list and a well-formed digest, and the summary showed no error. Read failures are recorded by the Postgres store and the service refuses with 503 and the reason, for `/context`, `/structure`, `/signals` and `/idea/`; a record projection that failed is a refusal too, rather than a package built from a graph that is stuck; and the summary reports whatever its counts hit.
