//! m75: Hale surface for filesystem ops.
//!
...
//!
//! These tests build a tiny Hale program per case, run it,
...
#[test]
fn hale_write_file_then_read_it_back_via_std_fs() {
    // Use std::fs from Rust to verify what Hale wrote — proves
    ...
    assert_eq!(on_disk, "hello from hale");
    let _ = std::fs::remove_file(&tmp);
}

#[test]
fn hale_read_file_returns_full_contents() {
...
#[test]
fn hale_round_trip_write_then_read() {
    // Hale writes, then reads back the same file in the same
...
#[test]
fn hale_file_size_returns_byte_count() {
...
#[test]
fn hale_file_exists_distinguishes_present_from_absent() {
...
#[test]
fn hale_extension_isolates_basename_last_dot() {
