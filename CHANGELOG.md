...
- **`hale dna` builds the organism's seed once per content fingerprint (GH #931).** `run` and `dev` rebuilt the organization's seed — a handful of project files plus the whole vendored core — on every start and every restart, 3.83s each time and 58.6s of the 212s the eleven slowest DNA integration tests spend. A build is a pure function of what it reads, so the host does it once per fingerprint and copies the binary after that: a first start binds the membrane in 3.91s, a second in 0.15s.
