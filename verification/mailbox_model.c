/* GenMC model of the lotus pinned-locus mailbox — LOCK-FREE rewrite.
 * GH issue #18 item 2 (race-completeness).
 *
 * FAITHFUL TRANSCRIPTION of the lock-free mailbox in `lotus_mailbox_*` /
 * `lotus_mpsc_ring_*` in crates/hale-codegen/runtime/lotus_arena.c — the
 * Vyukov bounded MPSC ring + the signal-only-when-parked wake handshake.
 ...
 *     posts <= CAP (it is a separate concern from the ring handoff + the
 *     parked-wake; the cross-thread backpressure path is validated under
 *     TSAN stress instead — GenMC's job here is the lock-free handoff and
 *     the missed-wakeup handshake);
 *   - the condvar is NOT modeled (GenMC has no condvars; they are a
 *     liveness mechanism). Instead the PARK is modeled by its essential
 *     safety core: the `parked` seq_cst flag + a seq_cst fence + the
 *     drainer's recheck, and the producer's publish + seq_cst fence +
 *     load(parked). This is exactly the Dekker / store-buffer (SB) shape
 *     that the real wake handshake reduces to. A `g_signaled` flag stands
 *     in for "the producer would cond_signal".
 *
 * What's kept EXACTLY: the Vyukov enqueue/dequeue seq protocol with its
 * acquire/release/relaxed orders, and the seq_cst store(parked) + fence
 * on the consumer / seq_cst fence + load(parked) on the producer. If
 * those orders change in the C, this model must change with them.
 *
 ...
 *   (4) NO MISSED WAKEUP          — if the drainer commits to parking
 *       (its recheck saw the ring empty), the producer MUST have observed
 *       parked==1 (so it would signal). A broken handshake — e.g. dropping
 *       either seq_cst fence — admits the SB outcome where the drainer
 *       parks AND the producer skips the signal, which this asserts away.
 *
...
/* ==================================================================== *
 * Phase 1 — ring safety: 2 concurrent producers + 1 consumer.
 * Checks (1) no loss, (2) no dup, (3) no torn value under concurrent
 * enqueue-CAS contention and the enqueue/dequeue handoff.
 * ==================================================================== */
static ring_t R1;
...
static void phase1_ring_safety(void) {
    ...
    g_seen[2] = 0;
    pthread_t pa, pb, dr;
    pthread_create(&dr, NULL, p1_drainer, NULL);
 ...
 *
 * One producer posts a single cell, then (fence) checks `parked` and
 * records whether it would cond_signal. One drainer attempts a dequeue;
 * if empty it sets parked=1, (fence) rechecks; if STILL empty it commits
 * to parking. The Dekker/SB property of the two seq_cst fences forbids
 * the outcome (producer saw parked==0) AND (drainer saw ring empty on its
 * recheck). So: drainer parked ⇒ producer signaled. That is precisely
 * "no missed wakeup" — the consumer never sleeps on a cell that no
 * producer will wake it for.
 * ==================================================================== */
static ring_t        R2;
static _Atomic int   g_parked;
static int           g_signaled;     /* producer observed parked → would signal */
static int           g_drainer_parked;
static int           g_drained_val;  /* -1 if the drainer parked instead */
...
static void *p2_drainer(void *_) {
    ...
    g_drainer_parked = 1;                    /* committed to sleeping */
    g_drained_val = -1;
    return NULL;
}
...
static void phase2_missed_wakeup(void) {
    ...
    g_drained_val = 0;
    pthread_t po, dr;
    pthread_create(&dr, NULL, p2_drainer, NULL);
    pthread_create(&po, NULL, p2_poster, NULL);
    pthread_join(po, NULL);
    pthread_join(dr, NULL);

    /* (4) NO MISSED WAKEUP: if the drainer committed to parking, the
     * producer must have seen parked==1 and signaled — otherwise the real
     * consumer would sleep forever on an already-posted cell. */
    if (g_drainer_parked) {
        assert(g_signaled == 1);
    } else {
