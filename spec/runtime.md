...
| `LOTUS_BUS_QUEUE_CAP=<N>` | Caps the cooperative bus dispatch queue at `N` cells (default 8192 ≈ 4.5 MB; floor 64; read once). When a **single-threaded** producer outruns its consumer and the queue hits the cap, the publisher *back-pressures* — it inline-drains the queue (runs the oldest handlers) to free space rather than growing without bound (GH #125). Every message is still delivered; resident memory stays bounded. The multithreaded (cross-pool) path still grows — running a handler on a foreign pool's thread to back-pressure there would violate the single-threaded-method invariant, so cross-pool backpressure is a follow-on. Lower the cap to tighten the bound (more frequent inline-drains); raise it to reduce drain bursts at the cost of resident memory. |
