...
| `std::term` | `is_tty(fd: Int) -> Bool` (POSIX `isatty` — probe whether an fd is a terminal, e.g. so a logger can decide on color without vendoring an FFI shim). Stage 1 of pond P4; `size` / `RawMode` / raw stdin follow per `notes/stdlib-term-primitives.md`. POSIX-only; non-tty / non-POSIX degrades soft. | `lotus_term_*` C primitives |
