...
//!
//! The bundled stdlib source (`runtime/stdlib.hl`) declares
...
//!   - runtime: a real Hale program that instantiates the
...
fn listener_binds_accepts_a_connection_and_exits_cleanly() {
    ...
    let bin = build_hale_binary("listener_accepts", &source);
...
fn listener_uses_default_host_when_omitted() {
    ...
    let bin = build_hale_binary("default_host", &source);
