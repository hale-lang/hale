...
| `concern.requested` / `concern.raised` | ledger | a source | a concern raised from a locus path about the part above it: what, severity, by; several concerns share one source, so a request carries its own `request` id and its answer names it in a field of its own (`… [request <id>]`), parsed and compared whole; one request is one concern, however often it is delivered |
