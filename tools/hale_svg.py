...
KEYWORDS = set(
    "accept approx as as_parent_for async avoiding await bindings birth birth_check block bound break bubble bulk bus cap capacity captures chunked claims closure connect const consume continue contract cooperative core cores count cover cross_machine dissolve domain drain drop duration during edges else epoch explicit export expose fail fallible fixed_cell fn for forbid group harmonic heap if impl import in indexed_by inferred inline interface intra_machine intra_process l3 let listen locus macro main match may_be_empty mode module mut node of on on_failure on_overflow or params payload persists_through perspective pinned placement pool prod projection publish quarantine reaches recognition release reorganize reperspective replicas require reserve resets_on resets_per_epoch resolution restart restart_in_place return rich ring_layout role run schedule seed self serialize_as serves shared_slab shm_ring slot_count spillover stable_when subject subscribe sum summary_only terminate tick tier topic topology trait type unix until via violate where while with within yield zero_copy".split()
)
