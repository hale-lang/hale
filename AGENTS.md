# AGENTS.md — Aperio for agents writing `.ap` programs

You are writing programs in Aperio. This file is the load-bearing
prompt: encoding first, then operational. Read it once before
you write any `.ap` code.

## The Design (recursive hypergraph)

Aperio operationalizes **The Design** — a substrate-invariant
recursive hypergraph — at the language substrate. Every rule
about how Aperio code is shaped traces back here. Quote node /
...
aperio ≜ operationalization(DESIGN, substrate=language)
map:
  locus               ↔ Σ
...
aperio.root: ⊥(language-graph) = DESIGN itself
```
...
Six shapes. Every well-written `.ap` program matches one. If
