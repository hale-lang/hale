...
| restore of one Work: an incarnation booted over the record the crash left re-proposes and the committer replays, so identities are the record's; cut before the attempt's admission, after it, after the claim, after the outcome, and after completion, each boot runs to the same completed record; settled work never reruns and an open claim under a recorded outcome is closed; a claimed unanswered attempt is redelivered with the same id and no new claim, once per incarnation, so a duplicate dispatch attaches; the old process's reply for the still-current attempt is accepted; a completed record rebooted twice gains no row | proven, card 12a |
| delivery across off-thread bindings; restart of retries and uncertain effects | not yet, cards 12b–13, 19 |
