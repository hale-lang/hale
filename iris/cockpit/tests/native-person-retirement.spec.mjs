...
test('native retirement transfers the complete reviewed work atomically and remains inspectable',async({page},info)=>{
  ...
  try{
    ...
    const invalid=command(s,plan.json.data);delete invalid.arguments.to;expect((await s.post(invalid)).status).toBe(400);expect(s.journal().head).toBe(before.head);
    const original=await s.current();expect((await s.post(s.command(original,'blair',randomUUID()))).status).toBe(200);const afterMove=s.journal();expect((await s.post(command(s,plan.json.data))).status).toBe(409);expect(s.journal().head).toBe(afterMove.head);
...
test('native retirement lost response survives API restart and revoked write grant with GET only',async({page})=>{
  const s=await startTaskService({taskPolicy:policy}),posts=[],errors=[];page.on('pageerror',e=>errors.push(e.message));page.on('request',r=>{if(r.method()==='POST')posts.push(r.url());});
  try{
    let delivered;
    await page.route('**/commands',async route=>{if(route.request().method()!=='POST')return route.continue();const response=await route.fetch();expect(response.status()).toBe(200);delivered=await response.json();await route.abort('failed');});
    ...
    const service=JSON.parse(fs.readFileSync(s.evidence+'/service.json'));const updated=policy(s.application,s.principal.name);updated.grants[0].retire=false;updated.grants[0].reassign=false;fs.writeFileSync(service.policy,JSON.stringify(updated));await s.restart();
    await page.reload();await expect(recovery(page)).toHaveAttribute('data-observation','observed');await expect(page.locator('.read-only')).toHaveText('Read only');expect(posts).toHaveLength(1);expect(s.journal().head).toBe(admitted.head);expect(s.requests.filter(r=>r.method==='POST'&&r.path.endsWith('/commands'))).toHaveLength(0);
