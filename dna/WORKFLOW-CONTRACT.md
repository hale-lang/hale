...
A human Work's case is its own handed Task, `<task>.s<i>.<key>` — the
Work's step index and member key under its Task, the child-Task form
above — stable across redeliveries and naming its parent Work by
construction (card 16).
...
**Valid results in the example.** `work.settled` for `t1/wf1/s0/a`
naming its current attempt; `workflow.settled` for `t1.s0.b` with
`parent_task: t1` and `spawning_step: t1/wf1/s0`. Not valid: a
settlement of `t1.s0.b` naming step `t1/wf1/s1`; an outcome for
`t1/wf1/s0/a/a0` recorded after `a1` was admitted; a second `done` for
`c1`. (Card 18 deleted `WorkDone` and `TaskSettled`; the rows are the
results.)
...
| delivery across off-thread bindings: an intent the host relays reaches `Dna.ask` over the organization's bound NATS connection (the nerves, GH #986) and the admission lands (`dna_run.rs`); from there every transition is a resident's own, so no step of an execution waits on a delivery across a binding (F.15 closed) | proven, cards 18–19 |
| the baseline, one oracle in every mode: the canonical example's registered member sets, each step activated and completed once, every Work settled once on an admitted attempt, one claim per attempt, the adapters' invocation count per attempt, the order of the record (C1 before its step, the grandchild before the child's step, the child before the root's step, D's step activated before D was claimed, the root last), and the tree reclaimed — under immediate, delayed and out-of-order replies, a duplicate of a settled reply and a reply from another identity (nobody's), a stale reply for a retried-past attempt, a failed grandchild (the ancestors fail, D never runs, the root settles failed after the outstanding leaf, which does not reopen its step) and a cancellation reaching the grandchild's leaf; in process (repeated completed runs leave nothing alive), over a git record (a restart mid-flight redelivers the pending attempts under their ids and records each redelivery, completed members do not run again, no step twice; the completed record rebooted gains no row and asks no adapter), over two memories (every fact of the tree is the ledger's, so a reconstruction keeps their order and a restart over it continues exactly), under a lease (a takeover fences this holder's replies; nothing of them is written; the run completes when the lease is back) | proven, card 19 (`workflow_conformance_test.hl`; the fixture and the runner each declare a unix listen binding, so every publish is queued as in a bound organism; each of the five steps, three executions and four leaves is checked on its own, never by an aggregate) |
| a real process-stop boundary: the public assembly (`Dna.run_workflow` over a `GitJournal`) killed with SIGKILL after C1's attempt was admitted, after it was claimed, and after its durable adapter recorded the effect but before the outcome landed — each restart resumes the admitted root, the attempt keeps its admission and its one claim, the redelivery is recorded once, the adapter's store is the outcome (C1 performed once), D runs once, the root settles last; the completed record rebooted runs nothing | proven, card 19 (`conformance/runner.hl`, driven by `workflow_conformance_test.hl`) |
| logical exactly-once apart from external-effect reconciliation: one `effect.requested` per attempt ever, one outcome, one `work.settled`; a durable adapter that recorded its effect is not asked again (its store answers); a pending performer that truthfully says it has not acted is asked again once by the incarnation that restored the attempt, under the same id, with `effect.redelivered` recorded — two invocations under one claim, and the record says so | proven, card 19 |

Native runs: `HALE_BIN=target/release/hale HALE_DNA_SOURCE=$PWD
target/release/hale test dna/tests/<fixture>` with the `hale` built from
this tree (the native suite, `cargo test -p hale-cli --test
dna_native_suite`, runs every `dna/tests/*_test.hl` the same way).
`workflow_conformance_test.hl` builds `dna/tests/conformance/runner.hl`
with that `hale` beside a copy of the core under its scratch and runs it
as a separate process; nothing is built in the checkout.
