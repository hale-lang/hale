...
- `dna/core/workflow_projection.hl`: `replay(projection, kind, entity, body)` applies one card 05 fact to plain rows and answers whether it was applied, is a replay of what is already held, or is refused. Applying reaches nothing outside the projection: the entry point carries `@no_syscall`, which the compiler checks through every method it reaches, and the locus holds no journal, gateway or model. The member join is by identity and never by counting — a Step registers its whole required set with each member's kind, a member answers once under the key it was registered as, and the Step completes only when every key has settled. A leaf member is answered by its Work settling and a child member by the execution it invoked settling into the step that spawned it; neither answers for the other. Refused: a Step never registered, a Work that is not a member of its Step (a Work is `<step>/<key>` and nothing longer), an attempt superseded by a later one, a settled Work, Step or execution settling again or differently, a child's settlement redirecting its answer, and a Step completing with members outstanding. Results are read by Work identity. `workflow_projection_test.hl` holds the oracle: `a` alone does not complete a Step requiring `{a, b}`, `a` answering again still does not, and `b` completes it once. Nothing admits or executes a workflow yet.
