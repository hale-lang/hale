...
use lotus_syntax::ast::{Program, TopDecl};
use lotus_syntax::{Diag, parse_source};
use lotus_types::{check_bundle, Bundle};
...
fn lt_files_in(dir: &Path) -> Vec<PathBuf> {
...
fn all_examples_parse_and_check() {
    ...
    for project in &projects {
        ...
        for file in &files {
            let source = fs::read_to_string(file).expect("read .lt");
