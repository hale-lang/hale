...
const taskPolicy = (application_id, name) => ({ format: 'dna.task-authority/1', application_id, owner: 'operations', members: ['alex', 'blair', 'casey', 'retired'], grants: [{ mode: 'local', name, reassign: true, recover: true }] });
...
test('lost native POST reply survives API restart and reload with GET-only exact-key recovery', async ({ page, service }) => {
  ...
  const committed = await nativeResponse; expect([200, 202]).toContain(committed.status); expect(committed.json.data.task.state).toBe('applied');
  await expect(recovery(page)).toBeVisible(); const before = await stored(page); expect(before).toHaveLength(1);
  expect(before[0].value).toMatchObject({ version: 5, operation: 'dna.task.reassign', request_id: savedCommand.request_id, target_id: service.task });
  ...
  await expect(recovery(page)).toHaveAttribute('data-observation', 'observed');
  expect((await stored(page))[0].value.request_id).toBe(savedCommand.request_id); expect(postCount).toBe(1);
  ...
  expect(lookup.status).toBe(200); expect(lookup.json.data.command_id).toBe(committed.json.data.command_id); expect(lookup.json.data.task).toEqual(committed.json.data.task);
});
...
test('native current-assignment conflict and retired/outside-policy recipients cannot reassign', async ({ page, service }, info) => {
  ...
  expect([200, 202]).toContain(alternate.status); expect(alternate.json.data.task.state).toBe('applied');
  const response = page.waitForResponse(r => r.request().method() === 'POST' && new URL(r.url()).pathname === service.prefix + '/commands');
  await confirmation(page).getByRole('button', { name: 'Confirm reassignment', exact: true }).click();
  const stale = await response; expect(stale.status()).toBe(409); expect((await stale.json()).error.code).toBe('stale_subject');
  ...
  const caps = await service.read(service.prefix + '/capabilities'); expect(caps.json.data.task_commands.recipients).not.toContain('retired');
  expect(caps.json.data.task_commands.recipients).not.toContain('outside');
  for (const recipient of ['retired', 'outside']) {
    const denied = await service.post(service.command(current, recipient, 'denied-' + recipient));
    expect(denied.status).toBe(403); expect(denied.json.error.code).toBe('forbidden');
  }
