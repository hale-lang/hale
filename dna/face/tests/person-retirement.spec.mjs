...
import { UNGATED, commandReceipt, commandReply, describeLine, receiptLine } from './command-wire.mjs';
...
async function fixture(page,options={}){
  ...
  const rows=()=>Array.from({length:script.count},(_,i)=>({id:'task/'+i,outcome:'Carry responsibility '+i,state:'handed',assignee:script.applied?script.command.payload.to:WHO,obligation:'handover',acceptance_digest:'',acceptance_bound:true,evidence_required:false,evidence_ref:'',waiting:'',assignment_digest:DIGEST,reassignment_supported:true,history:[{event_id:'1'.repeat(40),sequence:'3',kind:'task.handed',from:'',to:WHO,by:'org'},...(script.applied?[{event_id:(''+(i+2)).repeat(40),sequence:String(10+i),kind:'task.reassigned',from:WHO,to:script.command.payload.to,by:PRINCIPAL.name}]:[])]}));
  const receipt=()=>{const p=script.command.payload;return receiptLine(commandReply(commandReceipt({command_id:'command/'+p.request_id,request_id:p.request_id,application_id:APP,operation:'dna.person.retire',operation_version:'1',principal_mode:PRINCIPAL.mode,principal_name:PRINCIPAL.name,target_kind:'dna.person',target_id:p.person,subject_digest:DIGEST,fingerprint:'sha256:'+'f'.repeat(64),state:'succeeded',proposal_state:'',person:{state:'applied',from:WHO,to:p.to,event_id:EVENT,transferred:script.count}}),source()));};
  await page.route('**/api/hale/v1{,/**}',async route=>{
    ...
    if(url.pathname.endsWith('/capabilities'))return send(200,wrap({application_id:APP,principal:PRINCIPAL,read_only:true,reads:{tasks:true,people:true,organization:false,workflows:false,practices:false,reviews:false,definitions:false,knowledge:false},api:{transport:'unix',socket:'/run/scripted.sock',http:'/api/hale/v1/applications/'+APP+'/commands'}}));
...
test('person retirement shows the exact work, confirms once and observes the same retirement',async({page,host},info)=>{
  ...
  expect(s.posts[0]).toEqual({call:'PersonRetire',payload:{request_id:expect.any(String),person:WHO,subject_digest:DIGEST,to:TO}});
...
test('lost retirement reply recovers by GET after reload with write grant revoked',async({page,host})=>{
  ...
  await page.reload();await expect(recovery(page)).toHaveAttribute('data-observation','observed');expect(s.posts).toHaveLength(1);expect(s.gets).toContain(s.posts[0].payload.request_id);expect(s.errors).toEqual([]);
});
