...
- **Signal handling.** SIGINT / SIGTERM begin the whole-process
  ...
  expire, and then waits out the grace (`LOTUS_DRAIN_GRACE_MS`,
  default 5000) before exiting `128 + signal`. A second signal
  exits at once. Installed by the main prelude only in a program
  that reads `draining`; caught with `SA_RESTART`, never blocked,
  so spawned subprocesses keep the default disposition.
...
| `LOTUS_DRAIN_GRACE_MS=<ms>` | How long a SIGINT / SIGTERM drain may take before the runtime exits `128 + signal` (default 5000). Only in a program that reads `self.draining`; see `semantics.md` § "Drain cascade (whole-process)" (GH #1039). |
