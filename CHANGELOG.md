...
- `dna/core/performers.hl`, `dna/core/work_system.hl`, `dna/core/workflow_execution.hl`, `dna/core/assembly.hl`: recovery reconciles before it decides on a redelivery. `WorkSystem` carries a `Reconciler` beside its performers (`NoEvidence` by default; `IdempotentReplay` for a documented idempotent effect), asked about an attempt whose claim a dead incarnation left open: an adapter that acted hands back what it recorded, judged for whose it is, and that is the outcome — recorded, the claim closed, nothing performed twice; one that attests it never acted, or is documented idempotent, is redelivered, the row saying which; one that cannot say leaves the claim `effect.result unknown (reconciled after a restart: …)`, the existing rule-3 mark, appended exactly and under the fence, and the attempt waits visibly, replayed by nobody, until `hale dna effect resolve` says what happened, and then the resolution is recorded as the attempt's outcome so the Work settles. The assembly's restart rule leaves `attempt:` claims to the runtime. `workflow_reconcile_test.hl` holds the oracle: a durable fake adapter recording the invocation, the crash before the outcome, the restart completing without a second invocation; an adapter that cannot say, the unknown row, a resume that still waits, a person's resolution becoming the outcome; an idempotent adapter replayed; an adapter attesting it never acted redelivered; an adapter whose record names another performer recording nothing; the assembly leaving the attempt claim alone. `effect_outcomes_test.hl` stays green. The restore and retry fixtures declare their effect-free fakes idempotent. One flat leaf; no exactly-once promise, no compensation.
