...
| `Bool` | i1 | `true` / `false` |
| `Time` | i64 | Nanoseconds since the Unix epoch, UTC (GH #607, 2026-09-14). A literal is ISO-8601 between backticks — `` `2026-05-08T12:00:00Z` ``, an optional fraction of one to nine digits, an optional `Z`; an offset such as `+01:00` is a compile error, never read as UTC — and is parsed at check time. Arithmetic: `Time ± Duration → Time`, `Duration + Time → Time`, `Time − Time → Duration`; anything else with a `Time` operand is refused. Orders and compares as an instant. `to_string` / `println` render ISO-8601, the fraction only when it is not zero. Before the epoch is negative, not an error. On the wire and in a struct it is the same 8 bytes as `Int`; over FFI it is `i64`. `std::time::current()` reads the wall clock as one; `iso8601`, `parse_time`, `unix`, `nanos`, `from_nanos`, `time_from_unix` convert. Was a string-shaped placeholder until v0.20.0. |
| `Duration` | i64 | Nanoseconds. Suffix literals (`5s`, `100ms`). Arithmetic: `Duration ± Duration → Duration`; scalar scale `Int * Duration` (either order) and `Duration / Int` produce `Duration` (Crumb batch-3, 2026-07-28 — the runtime-computed-delay shape, `ms * 1ms`). `Duration * / % Duration` is a type error (ns² / dimensionless ratio have no unit-sane meaning; the diagnostic points at the scalar forms). |
