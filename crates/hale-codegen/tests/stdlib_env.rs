...
//!
//! These tests build small Hale programs that consume the
...
fn arg_returns_each_argv_entry_and_empty_for_out_of_range() {
    ...
    "#;
    let bin = build_hale("arg_indexed", src);
    let out = Command::new(&bin)
        .args(["first", "second"])
        .output()
        .expect("run");
...
fn var_returns_environment_value() {
    let src = r#"
        fn main() {
            let v = std::env::var("HALE_TEST_FOO");
            println("v=[", v, "]");
        }
    "#;
    let bin = build_hale("var_present", src);
    let out = Command::new(&bin)
        .env("HALE_TEST_FOO", "bar-baz")
        .output()
        .expect("run");
    let _ = std::fs::remove_file(&bin);
    assert!(out.status.success());
...
fn var_returns_empty_for_unset_variable() {
    let src = r#"
        fn main() {
            let v = std::env::var("HALE_DEFINITELY_UNSET_VARIABLE_XYZZY");
            println("v=[", v, "]");
        }
    "#;
    let bin = build_hale("var_absent", src);
    let out = Command::new(&bin)
        .env_remove("HALE_DEFINITELY_UNSET_VARIABLE_XYZZY")
        .output()
        .expect("run");
    let _ = std::fs::remove_file(&bin);
    assert!(out.status.success());
...
fn var_exists_distinguishes_present_from_absent() {
    let src = r#"
        fn main() {
            let p = std::env::var_exists("HALE_TEST_PRESENT");
            let a = std::env::var_exists("HALE_DEFINITELY_NEVER_SET");
            println("present=", p);
            println("absent=", a);
        }
    "#;
    let bin = build_hale("var_exists", src);
    let out = Command::new(&bin)
        .env("HALE_TEST_PRESENT", "x")
        .env_remove("HALE_DEFINITELY_NEVER_SET")
        .output()
        .expect("run");
    let _ = std::fs::remove_file(&bin);
    assert!(out.status.success());
