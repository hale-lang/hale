...
| `workflow_projection.hl` | `WorkflowProjection` and `replay` (`@no_syscall`): the state of one execution, read from its facts alone — `WorkView`, `StepView`, `TaskView`, `joined`, `pending`, `result_of`. The member join is by identity and by kind: a leaf answers when its Work settles, a child when the execution it invoked settles into the step that spawned it, each once |
