...
//!
//! Closes notes/aperio-friction.md `tcp-recv-string-strlen-truncates-binary`.
...
fn build_aperio(name: &str, source: &str) -> std::path::PathBuf {
    ...
    bin.push(format!("aperio_test_recvbytes_{}", name));
...
fn recv_bytes_preserves_nul_in_payload() {
    ...
    let bin = build_aperio("nul_preserved", &src);
...
fn from_string_round_trip_preserves_length() {
    ...
    "#;
    let bin = build_aperio("conv_round_trip", src);
...
fn bytes_at_out_of_range_returns_minus_one() {
    ...
    "#;
    let bin = build_aperio("at_oob", src);
    let out = Command::new(&bin).output().expect("run");
    let _ = std::fs::remove_file(&bin);
    assert!(out.status.success(), "stderr: {}", String::from_utf8_lossy(&out.stderr));
    let stdout = String::from_utf8_lossy(&out.stdout);
...
fn bytes_slice_returns_subrange() {
    ...
    "#;
    let bin = build_aperio("slice", src);
    let out = Command::new(&bin).output().expect("run");
    let _ = std::fs::remove_file(&bin);
    assert!(out.status.success(), "stderr: {}", String::from_utf8_lossy(&out.stderr));
    let stdout = String::from_utf8_lossy(&out.stdout);
...
fn from_string_then_send_bytes_round_trips_via_tcp() {
    ...
    let bin = build_aperio("from_string_send", &src);
    let out = Command::new(&bin).output().expect("run aperio");
    let received = got_rx
