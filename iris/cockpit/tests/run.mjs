...
const catalog = process.env.HALE_COCKPIT_CATALOG_BIN || '';
const knowledge = process.env.HALE_COCKPIT_KNOWLEDGE_BIN || '';
const commands = process.env.HALE_COCKPIT_COMMAND_BIN || '';
const workflows = process.env.HALE_COCKPIT_WORKFLOWS_BIN || '';
...
try {
  await validateProvider(catalog, 'HALE_COCKPIT_CATALOG_BIN', 'Definitions');
  await validateProvider(knowledge, 'HALE_COCKPIT_KNOWLEDGE_BIN', 'Knowledge');
  await validateProvider(commands, 'HALE_COCKPIT_COMMAND_BIN', 'Scripted command adapter');
  await validateProvider(workflows, 'HALE_COCKPIT_WORKFLOWS_BIN', 'Recorded workflows');
  await access(hale);
  await access(api);
  ...
  await run(process.execPath, [path.join(cockpit, 'node_modules/@playwright/test/cli.js'),
    ...
    ...(catalog ? { HALE_COCKPIT_CATALOG_BIN: catalog } : {}),
    ...(knowledge ? { HALE_COCKPIT_KNOWLEDGE_BIN: knowledge } : {}),
    ...(workflows ? { HALE_COCKPIT_WORKFLOWS_BIN: workflows } : {}),
    ...(commands ? { HALE_COCKPIT_COMMAND_BIN: commands } : {}),
