...
| `--target native\|wasm32\|<triple>` | The target to build for; `hale --list-targets` names every triple with its tier as seen from this host. `native` (default) and the host's own triple build the host platform; `wasm32` builds from any host. Every other triple is refused at argument parsing, before anything is built: a Windows triple as not buildable yet (GH #445), and another host's native triple as not buildable from this host — there is no cross-compilation (GH #969; GH #970 tracks adding it). |
